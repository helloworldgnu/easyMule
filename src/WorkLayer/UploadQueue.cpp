/* 
 * $Id: UploadQueue.cpp 11398 2009-03-17 11:00:27Z huby $
 * 
...
CUpDownClient* CUploadQueue::FindBestClientInQueue()
{
	...
	for (pos1 = waitinglist.GetHeadPosition();( pos2 = pos1 ) != NULL;)
	{
		...
		if ((::GetTickCount() - cur_client->GetLastUpRequest() > MAX_PURGEQUEUETIME) || !CGlobalVariable::sharedfiles->GetFileByID(cur_client->GetUploadFileID()) )
		{
...
void CUploadQueue::InsertInUploadingList(CUpDownClient* newclient) 
{
    ...
    // Add it last
    //  Comment UI
	CGlobalVariable::uploadBandwidthThrottler->AddToStandardList(uploadinglist.GetCount(), newclient->GetFileUploadSocket());
...
bool CUploadQueue::AddUpNextClient(LPCTSTR pszReason, CUpDownClient* directadd){
	...
	if (!directadd)
	{
        ...
        if(newclient)
		{
			...
			//  Comment UI
			//PostMessage(CGlobalVariable::m_hListenWnd,WM_FILE_UPDATE_UPLOADRANK,0, waitinglist.GetCount());
			UINotify(WM_FILE_UPDATE_UPLOADRANK,0, waitinglist.GetCount());
	...
	// statistic
	CKnownFile* reqfile = CGlobalVariable::sharedfiles->GetFileByID((uchar*)newclient->GetUploadFileID());
	if (reqfile)
		reqfile->statistic.AddAccepted();
		
	//  Comment UI
	//uint32 struPFilePeer[] = {(uint32)0,(uint32)newclient,(uint32)0};
	UINotify(WM_FILE_ADD_PEER,3,(LPARAM)newclient, newclient);
...
void CUploadQueue::UpdateActiveClientsInfo(DWORD curTick) {
    ...
    uint32 tempHighest = CGlobalVariable::uploadBandwidthThrottler->GetHighestNumberOfFullyActivatedSlotsSinceLastCallAndReset();;
	
    if(thePrefs.GetLogUlDlEvents() && CGlobalVariable::uploadBandwidthThrottler->GetStandardListSize() > (uint32)uploadinglist.GetSize()) {
        // debug info, will remove this when I'm done.
        AddDebugLogLine(false, _T("UploadQueue: Error! Throttler has more slots than UploadQueue! Throttler: %i UploadQueue: %i Tick: %i"), CGlobalVariable::uploadBandwidthThrottler->GetStandardListSize(), uploadinglist.GetSize(), ::GetTickCount());
...
void CUploadQueue::Process() {
	...
	uint64 sentBytes = 0;
	//  Comment UI
	sentBytes = CGlobalVariable::uploadBandwidthThrottler->GetNumberOfSentBytesSinceLastCallAndReset();
	avarage_dr_list.AddTail(sentBytes);
    m_avarage_dr_sum += sentBytes;

    CGlobalVariable::uploadBandwidthThrottler->GetNumberOfSentBytesOverheadSinceLastCallAndReset();
...
bool CUploadQueue::AcceptNewClient(uint32 curUploadSlots){
	...
	{
        //  Comment UI
		MaxSpeed = (uint16)(CGlobalVariable::lastCommonRouteFinder->GetUpload()/1024);
	}
    else
	{
#endif
...
#ifdef _ENABLE_USS
		|| !CGlobalVariable::lastCommonRouteFinder->AcceptNewClient()
#endif
		)
	{ 
...
void CUploadQueue::AddClientToQueue(CUpDownClient* client, bool bIgnoreTimelimit)
{
	...
	if (CGlobalVariable::IsConnected() 
		&& CGlobalVariable::IsFirewalled()
		&& !client->GetKadPort()
		&& client->GetDownloadState() == DS_NONE 
		&& !client->IsFriend()
		&& CGlobalVariable::serverconnect
		&& !CGlobalVariable::serverconnect->IsLocalServer(client->GetServerIP(),client->GetServerPort())
	...
	for (pos1 = waitinglist.GetHeadPosition();( pos2 = pos1 ) != NULL;)
	{
		...
		{	
			...
			client->UpdateUI(UI_UPDATE_QUEUELIST);
			//theApp.emuledlg->transferwnd->queuelistctrl.RefreshClient(client);
			return;			
		}
		else if ( client->Compare(cur_client) ) 
		{
			CGlobalVariable::clientlist->AddTrackClient(client); // in any case keep track of this client
	...
	else if ( client->GetIP()!=0 && ((int) CGlobalVariable::clientlist->GetClientsFromIP(client->GetIP())) >= iMaxSameIP )
	{
		if (thePrefs.GetVerbose())
	...
	// statistic values
	CKnownFile* reqfile = CGlobalVariable::sharedfiles->GetFileByID((uchar*)client->GetUploadFileID());
	if (reqfile)
   ...
   // cap the list
	...
	else
	{
		...
		//  Comment UI begin
		//uint32 struPFilePeer[] = {(uint32)0,(uint32)client,(uint32)true};
		//SendMessage(CGlobalVariable::m_hListenWnd,WM_FILE_ADD_PEER,4,(LPARAM)client);
		UINotify(WM_FILE_ADD_PEER,4,(LPARAM)client, client);
		//theApp.emuledlg->transferwnd->queuelistctrl.AddClient(client,true);
		UINotify(WM_FILE_UPDATE_UPLOADRANK,0,(LPARAM)waitinglist.GetCount());
		//theApp.emuledlg->transferwnd->ShowQueueCount(waitinglist.GetCount());
		//  Comment UI end
		client->SendRankingInfo();
	}
...
bool CUploadQueue::RemoveFromUploadQueue(CUpDownClient* client, LPCTSTR pszReason, bool updatewindow, bool earlyabort){
	...
	for (POSITION pos = uploadinglist.GetHeadPosition();pos != 0;){
		...
		if (client == curClient){
            ...
            CKnownFile* requestedFile = CGlobalVariable::sharedfiles->GetFileByID(client->GetUploadFileID());
            if(requestedFile != NULL) {
                requestedFile->UpdatePartsInfo();
            }
			CGlobalVariable::clientlist->AddTrackClient(client); // Keep track of this client
...
void CUploadQueue::RemoveFromWaitingQueue(POSITION pos, bool updatewindow){	
	...
	//  Comment UI begin
	if (updatewindow)
		//SendMessage(CGlobalVariable::m_hListenWnd,WM_FILE_REMOVE_PEER,4,(WPARAM)todelete);
		UINotify(WM_FILE_REMOVE_PEER,4,(LPARAM)todelete, todelete, true);
	/*if (updatewindow)
...
bool CUploadQueue::CheckForTimeOver(CUpDownClient* client){
	...
	if(client->HasCollectionUploadSlot()){
		CKnownFile* pDownloadingFile = CGlobalVariable::sharedfiles->GetFileByID(client->requpfileid);
...
VOID CALLBACK CUploadQueue::UploadTimer(HWND /*hwnd*/, UINT /*uMsg*/, UINT_PTR /*idEvent*/, DWORD /*dwTime*/)
{
	...
	try
	{
		...
		//  Comment UI
		CGlobalVariable::HandleDebugLogQueue();
        CGlobalVariable::HandleLogQueue();
        // Elandal: ThreadSafeLogging <--

#ifdef _ENABLE_USS
		// ZZ:UploadSpeedSense -->
		CGlobalVariable::lastCommonRouteFinder->SetPrefs(thePrefs.IsDynUpEnabled(), 
			CGlobalVariable::uploadqueue->GetDatarate(), 
        ...
        CGlobalVariable::uploadqueue->Process();
		CGlobalVariable::downloadqueue->Process();
		...
		//Added by thilon on 2007.03.12, for SpeedMeter
		/*if(counter%5 == 0)
		{
			uint32 uDownDatarate = CGlobalVariable::downloadqueue->GetDatarate();
			uint32 uUpDatarate = CGlobalVariable::uploadqueue->GetDatarate();

			theApp.emuledlg->m_mainTabWnd.m_SpeedMeterDlg.m_CtrlSpeedMeter.AddValues(uUpDatarate/1024, uDownDatarate/1024);
		...
		if (counter >= 10){
			...
			CGlobalVariable::clientcredits->Process();	// 13 minutes
			CGlobalVariable::serverlist->Process();		// 17 minutes
			CGlobalVariable::knownfiles->Process();		// 11 minutes
			//  Comment UI
			//theApp.friendlist->Process();		// 19 minutes
			CGlobalVariable::clientlist->Process();
			CGlobalVariable::sharedfiles->Process();
			...
			if( CGlobalVariable::serverconnect->IsConnecting() && !CGlobalVariable::serverconnect->IsSingleConnect() )
				CGlobalVariable::serverconnect->TryAnotherConnectionRequest();

			CGlobalVariable::listensocket->UpdateConnectionsStatus();
			//  Comment UI
			if (thePrefs.WatchClipboard4ED2KLinks()
				|| thePrefs.WatchClipboard4UrlLinks())
				theApp.SearchClipboard();

			if (CGlobalVariable::serverconnect->IsConnecting())
				CGlobalVariable::serverconnect->CheckForTimeout();
			...
			if (iupdateconnstats>=2) {
				iupdateconnstats=0;
				theStats.UpdateConnectionStats((float)CGlobalVariable::uploadqueue->GetDatarate()/1024, (float)CGlobalVariable::downloadqueue->GetDatarate()/1024);
			}
			// <-----khaos-

			//  Comment UI
			//if (theApp.emuledlg->activewnd == theApp.emuledlg->statisticswnd && theApp.emuledlg->IsWindowVisible() )
			if( theApp.emuledlg->m_mainTabWnd.IsTabActive(CMainTabWnd::TI_ADVANCE) && 
				theApp.emuledlg->m_mainTabWnd.m_dlgAdvance.IsTabActive(CAdvanceTabWnd::TI_STAT) )
			{
				// display graphs
				if (thePrefs.GetTrafficOMeterInterval()>0) {
					...
					{
						igraph=0;
						//theApp.emuledlg->statisticswnd->SetCurrentRate((float)(CGlobalVariable::uploadqueue->Getavgupload()/CGlobalVariable::uploadqueue->Getavg())/1024,(float)(CGlobalVariable::uploadqueue->Getavgdownload()/CGlobalVariable::uploadqueue->Getavg())/1024);
						//  Comment UI
						theApp.emuledlg->statisticswnd->SetCurrentRate((float)(CGlobalVariable::uploadqueue->GetDatarate())/1024,(float)(CGlobalVariable::downloadqueue->GetDatarate())/1024);
						//CGlobalVariable::uploadqueue->Zeroavg();
					}
				}				
            ...
            CGlobalVariable::uploadqueue->UpdateDatarates();
            
			//ADDED by fengwen on 2007/04/11	<begin> :
			theStatForServer.RecordCurrentRate((float)(CGlobalVariable::uploadqueue->GetDatarate())/1024,(float)(CGlobalVariable::downloadqueue->GetDatarate())/1024);
			//ADDED by fengwen on 2007/04/11	<end> :

            //save rates every second
			theStats.RecordRate();
			// mobilemule sockets
			CGlobalVariable::mmserver->Process();
...
#ifdef _ENABLE_USS
            bool gotEnoughHosts = CGlobalVariable::clientlist->GiveClientsForTraceRoute();
			...
			if (sec>=5)
			{
				...
				sec = 0;
				CGlobalVariable::listensocket->Process();
				//  Comment UI
				
				PostMessage(CGlobalVariable::m_hListenWnd, WM_SHOW_TRANSFER_RATE, 0, 0);
			...
			if (statsave>=60) {
				...
				if (thePrefs.GetWSIsEnabled())
					CGlobalVariable::webserver->UpdateSessionCount();

				CGlobalVariable::serverconnect->KeepConnectionAlive();

#ifdef _SUPPORT_MEMPOOL
				theApp.m_pMemoryPool->CheckToRelease(); 
#endif
				CGlobalVariable::serverconnect->CheckStatTimeOut(); // VC-SearchDream[2007-06-14]: Report to Stat Server
			}
...
void CUploadQueue::ReSortUploadSlots(bool force) {
    ...
    if(force ||  curtick - m_dwLastResortedUploadSlots >= 10*1000) {
		...
		//  Comment UI
        CGlobalVariable::uploadBandwidthThrottler->Pause(true);
        ...
        while (ulpos != NULL) {
            ...
            //  Comment UI
			CGlobalVariable::uploadBandwidthThrottler->RemoveFromStandardList(cur_client->socket);
            CGlobalVariable::uploadBandwidthThrottler->RemoveFromStandardList((CClientReqSocket*)cur_client->m_pPCUpSocket);
        ...
        //  Comment UI
		CGlobalVariable::uploadBandwidthThrottler->Pause(false);
    }
}
