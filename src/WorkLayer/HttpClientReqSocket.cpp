/* 
 * $Id: HttpClientReqSocket.cpp 4887 2008-03-04 10:00:54Z huby $
 * 
...
CHttpClientReqSocket::CHttpClientReqSocket(CUpDownClient* client) : CClientReqSocket(client)
{
...
void CHttpClientReqSocket::OnConnect(int nErrorCode)
{
	if (NULL != GetClient())
	{
		if (WSAENETUNREACH == nErrorCode || WSAETIMEDOUT == nErrorCode)
		{
...
void CHttpClientReqSocket::DataReceived(const BYTE* pucData, UINT uSize)
{
	...
	if (!bResult && !IsDeleteThis())
	{
		...
		// In case this socket is attached to an CHttpClient, we are dealing with the real CUpDownClient here
		...
		// In case this socket is attached to an CHttpClient, we are disconnecting the real CUpDownClient here
...
bool CHttpClientDownSocket::ProcessHttpResponseBody(const BYTE* pucData, UINT size)
{
	try
	{
		...
		GetClient()->ProcessRawData(pucData, size);
	}
	catch (CException* e)
	{
		e->Delete();
