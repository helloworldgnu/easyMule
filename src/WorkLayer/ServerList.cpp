/* 
 * $Id: ServerList.cpp 11398 2009-03-17 11:00:27Z huby $
 * 
...
bool CServerList::AddServerMetToList(const CString& strFile, bool bMerge) 
{
	...
	try{
		...
		for (UINT j = 0; j < fservercount; j++)
		{
			...
			//{begin} VC-dgkang 2008年8月14日
			if(thePrefs.GetFilterServerByIP())
			{
				if (CGlobalVariable::ipfilter->IsFiltered(newserver->GetIP()))
				{
					if (thePrefs.GetLogFilteredIPs())
						AddDebugLogLine(false, _T("Filtered server \"%s\" (IP=%s) - IP filter (%s)"), newserver->GetListName(), ipstr(newserver->GetIP()),
						    CGlobalVariable::ipfilter->GetLastHit());
					delete newserver;
					continue;
				}
			}
