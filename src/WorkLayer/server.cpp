/* 
 * $Id: server.cpp 11398 2009-03-17 11:00:27Z huby $
 * 
...
void CServer::ResetIP2Country(){
	m_structServerCountry = CGlobalVariable::ip2country->GetCountryFromIP(ip);
}
...
CServer::CServer(const ServerMet_Struct* in_data)
{
	...
	m_uLowIDUsers = 0;
	challenge = 0;
	m_structServerCountry = CGlobalVariable::ip2country->GetCountryFromIP(ip); //EastShare - added by AndCycle, IP to Country
	m_dwServerKeyUDP = 0;
...
uint32 CServer::GetServerKeyUDP(bool bForce) const{
	if (m_dwIPServerKeyUDP != 0 && m_dwIPServerKeyUDP == CGlobalVariable::GetPublicIP() || bForce)
...
void CServer::SetServerKeyUDP(uint32 dwServerKeyUDP){
	ASSERT( CGlobalVariable::GetPublicIP() != 0 || dwServerKeyUDP == 0 );
	m_dwServerKeyUDP = dwServerKeyUDP;
	m_dwIPServerKeyUDP = CGlobalVariable::GetPublicIP();
}
