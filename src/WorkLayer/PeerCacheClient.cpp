/* 
 * $Id: PeerCacheClient.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
void CPeerCacheUpSocket::DetachFromClient()
{
	if (GetClient())
	{
        if (GetClient()->m_pPCUpSocket == this) {
			GetClient()->m_pPCUpSocket = NULL;
            CGlobalVariable::uploadBandwidthThrottler->RemoveFromStandardList(this);
        }
	}
}
...
UINT CUpDownClient::ProcessPeerCacheUpHttpRequest(const CStringAArray& astrHeaders)
{
	...
	CKnownFile* pUploadFile = CGlobalVariable::sharedfiles->GetFileByID(aucUploadFileID);
	if (pUploadFile == NULL){
		DebugHttpHeaders(astrHeaders);
...
bool CUpDownClient::SendHttpBlockRequests()
{
	...
	sockAddr.sin_port = htons( CGlobalVariable::m_pPeerCache->GetCachePort() );
	sockAddr.sin_addr.S_un.S_addr = CGlobalVariable::m_pPeerCache->GetCacheIP();
...
bool CUpDownClient::SendPeerCacheFileRequest()
{
	...
	data.WriteUInt8(5);
	CTag tagCacheIP(PCTAG_CACHEIP, CGlobalVariable::m_pPeerCache->GetCacheIP());
	...
	CTag tagPublicIP(PCTAG_PUBLICIP, CGlobalVariable::GetPublicIP());
	tagPublicIP.WriteNewEd2kTag(&data);
	CTag tagCachePort(PCTAG_CACHEPORT, CGlobalVariable::m_pPeerCache->GetCachePort());
...
bool CUpDownClient::ProcessPeerCacheQuery(const uchar* packet, UINT size)
{
	...
	CKnownFile* pUploadFile = CGlobalVariable::sharedfiles->GetFileByID(aucFileHash);
	if (pUploadFile == NULL){
		if (thePrefs.GetVerbose())
	...
	CGlobalVariable::uploadBandwidthThrottler->AddToStandardList(0, m_pPCUpSocket);
...
bool CUpDownClient::ProcessPeerCacheAnswer(const uchar* packet, UINT size)
{
	...
	CGlobalVariable::m_pPeerCache->DownloadAttemptStarted();
	ASSERT( m_ePeerCacheDownState == PCDS_WAIT_CACHE_REPLY );
	return true;
}
...
void CUpDownClient::SetPeerCacheUpState(EPeerCacheUpState eState)
{
	if (m_ePeerCacheUpState != eState)
	{
        ...
        CGlobalVariable::uploadqueue->ReSortUploadSlots(true);
