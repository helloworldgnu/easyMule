/* 
 * $Id: PeerCacheFinder.cpp 4830 2008-02-22 08:04:43Z soarchin $
 * 
...
bool CPeerCacheFinder::IsCacheAvailable() const { 
	...
	return m_PCStatus == PCS_READY && CGlobalVariable::GetPublicIP() != 0 && m_nPCPort != 0; 
}
	...
	if (CGlobalVariable::IsRunning()){
		if (Valdite()){
			...
			if (m_pOwner->m_PCStatus == PCS_VALDITING){
				//  Comment UI
				DEBUG_ONLY(CGlobalVariable::QueueDebugLogLine(false, _T("PeerCache: Valditing .p2pinfo succeeded")));
				DEBUG_ONLY(CGlobalVariable::QueueLogLine(false, GetResString(IDS_PEERCACHE_ENABLED)));
...
bool CPCValditeThread::Valdite(){
	...
	if (file && nIFileSize > SIGNATURELENGTH){
		...
		if( !bContentCheckFailed && HttpQueryInfo(*file, HTTP_QUERY_RAW_HEADERS_CRLF, achBuffer, &nBufferSize, NULL) ){
			...
			///////***** Own version check
			//  Comment UI
			//if (!m_pOwner->IsClientPCCompatible(CClientVersionInfo(CGlobalVariable::m_nVersionMjr, CGlobalVariable::m_nVersionMin, CGlobalVariable::m_nVersionUpd, CGlobalVariable::m_nVersionBld, SO_EMULE))){
			//	bContentCheckFailed = true;
				//  Comment UI
