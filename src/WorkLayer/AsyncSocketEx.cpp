/*
 * $Id: AsyncSocketEx.cpp 11398 2009-03-17 11:00:27Z huby $
 * 
...
CAsyncSocketEx::~CAsyncSocketEx()
{
	...
	{
		CGlobalVariable::natthread->RemoveSocket(this);
	}
...
int CAsyncSocketEx::Receive(void* lpBuf, int nBufLen, int nFlags /*=0*/)
{
	...
	if(m_bUseNat)
	{
		return CGlobalVariable::natthread?CGlobalVariable::natthread->Receive(this, lpBuf, nBufLen) : -1;
		//return -1;
	}
#ifndef NOLAYERS
	if (m_pFirstLayer)
...
int CAsyncSocketEx::Send(const void* lpBuf, int nBufLen, int nFlags /*=0*/)
{
	...
	if(m_bUseNat)
	{
		return CGlobalVariable::natthread?CGlobalVariable::natthread->Send(this, lpBuf, nBufLen):-1;
		//return -1;
	}
#ifndef NOLAYERS
	if (m_pFirstLayer)
...
BOOL CAsyncSocketEx::Connect(const SOCKADDR* lpSockAddr, int nSockAddrLen)
{
	...
	if(m_bUseNat)
	{
		//if(CGlobalVariable::natthread->IsSocketConnected(this))
		//{
		...
		CUpDownClient * pClient = CGlobalVariable::clientlist->FindClientByUserHash((uchar*)lpSockAddr);
		...
		//if(CGlobalVariable::natthread) CGlobalVariable::natthread->Connect(this, (uchar*)lpSockAddr);
		if(CGlobalVariable::natthread && m_pTraverseFac) 
		{
			CGlobalVariable::natthread->Connect(this, (uchar*)lpSockAddr, m_pTraverseFac);
			return TRUE;
		}
		return FALSE;
	}
