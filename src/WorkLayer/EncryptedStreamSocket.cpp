/* 
 * $Id: EncryptedStreamSocket.cpp 4830 2008-02-22 08:04:43Z soarchin $
 * 
		...
		case ECS_UNKNOWN:{
			...
			if (!bNormalHeader){
				...
				if (thePrefs.IsClientCryptLayerRequired()){
					...
					if (thePrefs.IsClientCryptLayerRequiredStrict() || (!CGlobalVariable::serverconnect->AwaitingTestFromIP(sockAddr.sin_addr.S_un.S_addr)
						&& !CGlobalVariable::clientlist->IsKadFirewallCheckIP(sockAddr.sin_addr.S_un.S_addr)) )
					{
		...
		while (m_NegotiatingState != ONS_COMPLETE && m_nReceiveBytesWanted > 0){		
			...
			switch (m_NegotiatingState){
				...
				case ONS_BASIC_CLIENTA_PADDING:{
					...
					const EEncryptionMethods bySelectedEncryptionMethod = ENM_OBFUSCATION; // we do not support any further encryption in this version, so no need to look which the other client preferred
