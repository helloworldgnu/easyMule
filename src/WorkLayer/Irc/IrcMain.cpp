/* 
 * $Id: IrcMain.cpp 7701 2008-10-15 07:34:41Z huby $
 * 
			...
			case CED2KLink::kFile:
				{
					...
					//CGlobalVariable::downloadqueue->AddFileLinkToDownload(pFileLink);
					CmdFuncs::AddFileLinkToDownload(pFileLink);
		...
		if( sCommand == _T("PRIVMSG") )
		{
			...
			if( sRawMessage.Mid(iIndex, 2) == _T(":\001") )
			{
				...
				if( sRawMessage.Mid(iIndex, 9) == _T(":\001VERSION") )
				{
					//Get client version.
					iIndex += 9;
					m_sVersion = _T("eMule") + CGlobalVariable::GetCurVersionLong() + _T(Irc_Version);
					CString sBuild;
				...
				if( sRawMessage.Mid(iIndex, 11) == _T(":\001RQSFRIEND") )
				{
					...
					if(CGlobalVariable::serverconnect->IsConnected())
					{
						...
						sIP.Format( _T("%i"), CGlobalVariable::serverconnect->GetCurrentServer()->GetIP());
						sPort.Format( _T("%i"), CGlobalVariable::serverconnect->GetCurrentServer()->GetPort());
					}
					else
					{
						//Not on a server.
					...
					//Create our response.
					CString sBuild;
					sBuild.Format( _T("PRIVMSG %s :\001REPFRIEND eMule%s%s|%s|%u:%u|%s:%s|%s|\001"), sTarget, CGlobalVariable::GetCurVersionLong(), Irc_Version, sVerify, CGlobalVariable::IsFirewalled() ? 0 : CGlobalVariable::GetID(), thePrefs.GetPort(), sIP, sPort, md4str(thePrefs.GetUserHash()));
					m_pIRCSocket->SendString( sBuild );
...
void CIrcMain::Connect()
{
	try
	{
		...
		m_sVersion = _T("eMule") + CGlobalVariable::GetCurVersionLong() + (CString)Irc_Version;
