/* 
 * $Id: MMServer.cpp 9297 2008-12-24 09:55:04Z dgkang $
 * 
...
void CMMServer::ProcessStatusRequest(CMMSocket* sender, CMMPacket* packet){
	...
	packet->WriteShort((uint16)CGlobalVariable::uploadqueue->GetDatarate()/100);
	packet->WriteShort((uint16)((thePrefs.GetMaxGraphUploadRate(true)*1024)/100));
	packet->WriteShort((uint16)CGlobalVariable::downloadqueue->GetDatarate()/100);
	packet->WriteShort((uint16)((thePrefs.GetMaxGraphDownloadRate()*1024)/100));
	packet->WriteByte((uint8)CGlobalVariable::downloadqueue->GetDownloadingFileCount());
	packet->WriteByte((uint8)CGlobalVariable::downloadqueue->GetPausedFileCount());
	...
	if (CGlobalVariable::serverconnect->IsConnected()){
		if(CGlobalVariable::serverconnect->IsLowID())
			...
			packet->WriteByte(2);
		if (CGlobalVariable::serverconnect->GetCurrentServer() != NULL){
			packet->WriteInt(CGlobalVariable::serverconnect->GetCurrentServer()->GetUsers());
			packet->WriteString(CGlobalVariable::serverconnect->GetCurrentServer()->GetListName());
		}
		else{
			packet->WriteInt(0);
			packet->WriteString("");
...
void CMMServer::ProcessFileListRequest(CMMSocket* sender, CMMPacket* packet){
	...
	nCount = (CGlobalVariable::downloadqueue->GetFileCount() > m_nMaxDownloads)? m_nMaxDownloads : CGlobalVariable::downloadqueue->GetFileCount();
	m_SentFileList.SetSize(nCount);
	packet->WriteByte((uint8)nCount);
	for (int i = 0; i != nCount; i++){
		// while this is not the fastest method the trace this list, it's not timecritical here
		CPartFile* cur_file = CGlobalVariable::downloadqueue->GetFileByIndex(i);
		if (cur_file == NULL){
			delete packet;
...
void CMMServer::ProcessFileCommand(CMMData* data, CMMSocket* sender){
	...
	if (byFileIndex >= m_SentFileList.GetSize()
		|| !CGlobalVariable::downloadqueue->IsPartFile(m_SentFileList[byFileIndex]))
	{
		CMMPacket* packet = new CMMPacket(MMP_GENERALERROR);
		sender->SendPacket(packet);
		ASSERT ( false );
		...
		case MMT_CANCEL:{
			switch(selFile->GetStatus()) { 
				...
				case PS_PAUSED:
					...
					break;
				default:
                    CGlobalVariable::downloadqueue->StartNextFileIfPrefs(selFile->GetCategory());
					selFile->DeleteFile(); 
			}
			break;
		}
...
void  CMMServer::ProcessSearchRequest(CMMData* data, CMMSocket* sender){
	...
	CGlobalVariable::searchlist->NewSearch(NULL, Params.strFileType, MMS_SEARCHID, Params.eType, Params.strExpression, true);
	...
	CGlobalVariable::serverconnect->SendPacket(searchpacket,true);
	char buffer[500];
...
#define MAXRESULTS	20
	...
	if (bTimeOut){
	...
	else if (CGlobalVariable::searchlist->GetFoundFiles(MMS_SEARCHID) == 0){
		CMMPacket* packet = new CMMPacket(MMP_SEARCHANS);
		...
		UINT results = CGlobalVariable::searchlist->GetFoundFiles(MMS_SEARCHID);
		...
		for (UINT i = 0; i < results; i++){
			CSearchFile* cur_file = CGlobalVariable::searchlist->DetachNextFile(MMS_SEARCHID);
		...
		m_pPendingCommandSocket->SendPacket(packet);
		CGlobalVariable::searchlist->RemoveResults(MMS_SEARCHID);
	}
	m_pPendingCommandSocket = NULL;
}
...
void  CMMServer::ProcessDownloadRequest(CMMData* data, CMMSocket* sender){
	...
	//CGlobalVariable::downloadqueue->AddSearchToDownload(todownload, 2, 0);
	CmdFuncs::AddSearchToDownload(todownload, 2, 0);
	//MODIFIED by fengwen on 2007/02/09	<end> :
	CMMPacket* packet = new CMMPacket(MMP_DOWNLOADANS);
	if (CGlobalVariable::downloadqueue->GetFileByID(todownload->GetFileHash()) != NULL){
		packet->WriteByte(MMT_OK);
	}
	else{
...
void  CMMServer::ProcessPreviewRequest(CMMData* data, CMMSocket* sender){
	...
	if (byFileType == MMT_PARTFILFE){
		...
		|| !CGlobalVariable::knownfiles->IsKnownFile(m_SentFinishedList[byFileIndex]))
		{
			bError = true;	
		}
		else
...
VOID CALLBACK CMMServer::CommandTimer(HWND /*hwnd*/, UINT /*uMsg*/, UINT_PTR /*idEvent*/, DWORD /*dwTime*/)
{
	...
	try
	{
		KillTimer(0,CGlobalVariable::mmserver->h_timer);
		CGlobalVariable::mmserver->h_timer = 0;
		switch(CGlobalVariable::mmserver->m_byPendingCommand){
			case MMT_SDPC:{
				HANDLE hToken; 
			...
			case MMT_SDEMULE:
				...
				break;
			case MMT_SEARCH:
				CGlobalVariable::mmserver->SearchFinished(true);
				break;
			case MMT_PREVIEW:
				CGlobalVariable::mmserver->PreviewFinished(NULL,0);
				break;
		}
	}
...
void  CMMServer::ProcessStatisticsRequest(CMMData* data, CMMSocket* sender){
	...
	CArray<UpDown>* rawData = CGlobalVariable::webserver->GetPointsForWeb();
