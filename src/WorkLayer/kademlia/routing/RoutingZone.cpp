/* 
 * $Id: RoutingZone.cpp 9297 2008-12-24 09:55:04Z dgkang $
 * 
...
CRoutingZone::~CRoutingZone()
{
	...
	if ((m_pSuperZone == NULL) && (m_sFilename.GetLength() > 0))
	{
		...
		PostMessage( CGlobalVariable::m_hListenWnd, WM_KAD_HIDECONTACTS, 0, 0 ); 
		WriteFile();
	}
	...
	if (m_pSuperZone == NULL)
		PostMessage( CGlobalVariable::m_hListenWnd, WM_KAD_SHOWCONTACTS, 0, 0 ); 
}
			...
			if (uNumContacts != 0 && uNumContacts * 25 <= (file.GetLength() - file.GetPosition()))
			{
				...
				while ( uNumContacts )
				{
					...
					if( byType < 4)
					{
						...
						{
							if (::CGlobalVariable::ipfilter->IsFiltered(uhostIP))
							{
								if (::thePrefs.GetLogFilteredIPs())
									AddDebugLogLine(false, _T("Ignored kad contact (IP=%s:%u)--read known.dat -- - IP filter (%s)") , ipstr(uhostIP), uUDPPort, CGlobalVariable::ipfilter->GetLastHit());
							}
...
bool CRoutingZone::Add(const CUInt128 &uID, uint32 uIP, uint16 uUDPPort, uint16 uTCPPort, uint8 uVersion, CKadUDPKey cUDPKey, bool& bIPVerified, bool bUpdate, bool bFromNodesDat, bool bFromHello)
{
	...
	if (::IsGoodIPPort(uhostIP, uUDPPort))
	{
		if (!CGlobalVariable::ipfilter->IsFiltered(uhostIP) && !(uUDPPort == 53 && uVersion <= KADEMLIA_VERSION5_48a)  /*No DNS Port without encryption*/) {
			...
			AddDebugLogLine(false, _T("Ignored kad contact (IP=%s:%u) - IP filter (%s)"), ipstr(uhostIP), uUDPPort, ::CGlobalVariable::ipfilter->GetLastHit());
...
bool CRoutingZone::Add(CContact* pContact, bool& bUpdate, bool& bOutIPVerified)
{
	...
	else
	{
		...
		if (pContactUpdate)
		{
			if(bUpdate)
			{
				if (pContactUpdate->GetUDPKey().GetKeyValue(CGlobalVariable::GetPublicIP(false)) != 0
					&& pContactUpdate->GetUDPKey().GetKeyValue(CGlobalVariable::GetPublicIP(false)) != pContact->GetUDPKey().GetKeyValue(CGlobalVariable::GetPublicIP(false)))
				{
					...
					DebugLogWarning(_T("Kad: Sender (%s) tried to update contact entry but failed to provide the proper sender key (Sent Empty: %s) for the entry (%s) - denying update")
						, ipstr(ntohl(pContact->GetIPAddress())), pContact->GetUDPKey().GetKeyValue(CGlobalVariable::GetPublicIP(false)) == 0 ? _T("Yes") : _T("No")
					...
					&& pContactUpdate->GetReceivedHelloPacket())
				{
						...
						&& pContactUpdate->GetVersion() == pContact->GetVersion() && pContactUpdate->GetUDPPort() == pContact->GetUDPPort())
					{
						...
						SendMessage(CGlobalVariable::m_hListenWnd, WM_KAD_CONTACTREF, 0, (LPARAM)pContactUpdate);
					...
					if (pContact->GetUDPKey().GetKeyValue(CGlobalVariable::GetPublicIP(false)) == 0){
		...
		else if (m_pBin->GetRemaining())
		{
			...
			if(m_pBin->AddContact(pContact))
			{
				...
				//  Comment UI
				if( SendMessage(CGlobalVariable::m_hListenWnd, WM_KAD_CONTACTADD, 0, (LPARAM)pContact) )
...
bool CRoutingZone::VerifyContact(const CUInt128 &uID, uint32 uIP){
	...
	if (pContact == NULL){
		...
		if (pContact->IsIpVerified())
			...
			SendMessage(CGlobalVariable::m_hListenWnd, WM_KAD_CONTACTREF, 0, (LPARAM)pContact);
		}
		return true;
	}
}
