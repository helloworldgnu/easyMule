/* 
 * $Id: Kademlia.h 9297 2008-12-24 09:55:04Z dgkang $
 * 
...
namespace Kademlia
{
	...
	class CKademlia
	{
		public:
			...
			static void	Process();
			static bool	InitUnicode(LPBYTE pRes, DWORD dwSize);
		...
		private:
			...
			//static time_t m_tNextUpdateWholeKBucket; //COMMENTED by VC-fengwen on 2007/11/19 : 快速填充一次之后就不用快速填充了。
			...
			//ADDED by VC-fengwen on 2007/11/19 <begin> : 由于udp也耗路由器的连接数。只有设置了快速填充才执行快速填充。 快速填充一次之后就不用快速填充了。
			static bool m_bFastNodesInit;
			static bool m_bAlreadyFastNodesInit;
			//ADDED by VC-fengwen on 2007/11/19 <end> : 由于udp也耗路由器的连接数。只有设置了快速填充才执行快速填充。 快速填充一次之后就不用快速填充了。
	};
}
