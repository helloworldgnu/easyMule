...
void CUDPFirewallTester::Reset(){
	...
	m_dwTestStart = 0;
	m_dwLastSucceededTime = 0;
	CSearchManager::CancelNodeFWCheckUDPSearch(); // cancel firewallnode searches if any are stillactive
...
void CUDPFirewallTester::QueryNextClient(){ // try the next available client for the firewallcheck
	...
	while (!m_liPossibleTestClients.IsEmpty()){
		...
		if (ntohl(curContact.GetIPAddress()) == CGlobalVariable::GetPublicIP() || curContact.GetClientID().CompareTo(CKademlia::GetPrefs()->GetKadID()) == 0)
			continue;
		...
		if (!bAlreadyRequested && CKademlia::GetRoutingZone()->GetContact(curContact.GetIPAddress(), 0, false) == NULL){
			...
			if (CGlobalVariable::clientlist->DoRequestFirewallCheckUDP(curContact)){
