/* 
 * $Id: Kademlia.cpp 9297 2008-12-24 09:55:04Z dgkang $
 * 
...
void CKademlia::Start(CPrefs *pPrefs)
{
	try
	{
		...
		//MODIFIED by VC-fengwen on 2007/11/19 <begin> :
			//m_tNextUpdateWholeKBucket = time(NULL);	//ADDED by VC-fengwen 2007/08/27 : restart后重新更新K桶，以避免upnp成功后restart kad，中断了更新K桶的操作。
		m_bAlreadyFastNodesInit = false;
		//MODIFIED by VC-fengwen on 2007/11/19 <end> :
		
		//MODIFIED by VC-fengwen on 2007/11/19 <begin> : 由于udp会占用路由器的连接数，所以根据设置来决定，是否快速填充K桶。
		...
		if (m_bFastNodesInit)
			m_tNextSelfLookup = time(NULL);
		else
			// Force a FindNodeComplete within the first 3 minutes.
			m_tNextSelfLookup = time(NULL) + MIN2S(3);
		//MODIFIED by VC-fengwen on 2007/11/19 <end> : 由于udp会占用路由器的连接数，所以根据设置来决定，是否快速填充K桶。
...
void CKademlia::Process()
{
	...
	if( bUpdateUserFile )
	{
		if( uMaxUsers != m_pInstance->m_pPrefs->GetKademliaUsers())
		{
			...
			SendMessage(CGlobalVariable::m_hListenWnd, WM_SHOW_USERCOUNT, 0, 0);
		}
	}
...
bool CKademlia::StartFindBuddy()
{
	if( m_pInstance && m_pInstance->m_pPrefs )
	{
		...
		if( CGlobalVariable::clientlist->GetBuddyStatus()== Disconnected && Kademlia::CKademlia::GetPrefs()
...
bool CKademlia::m_bFastNodesInit = true;
bool CKademlia::m_bAlreadyFastNodesInit = false;
//time_t CKademlia::m_tNextUpdateWholeKBucket = 0; //COMMENTED by VC-fengwen on 2007/11/19 : 快速填充一次之后就不用快速填充了。
void CKademlia::Process_UpdateWholeKBucket()
{
	//MODIFIED by VC-fengwen on 2007/11/19 <begin> : 只有设置了快速填充才执行快速填充。 快速填充一次之后就不用快速填充了。
		//time_t tNow = time(NULL);
	...
	if (m_bFastNodesInit && !m_bAlreadyFastNodesInit)
	{
		UpdateWholeKBucket();
		m_bAlreadyFastNodesInit = true;
	}
	//MODIFIED by VC-fengwen on 2007/11/19 <end> : 只有设置了快速填充才执行快速填充。 快速填充一次之后就不用快速填充了。
}
