/* 
 * $Id: KademliaUDPListener.cpp 9297 2008-12-24 09:55:04Z dgkang $
 * 
...
void CKademliaUDPListener::FirewalledCheck(uint32 uIP, uint16 uUDPPort, CKadUDPKey senderUDPKey, uint8 byKadVersion)
{
	...
	CGlobalVariable::clientlist->AddKadFirewallRequest(ntohl(uIP));
}
...
void CKademliaUDPListener::Process_KADEMLIA_RES (const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort)
{
	...
	try
	{
		for (uint16 iIndex=0; iIndex<uNumContacts; iIndex++)
		{
			...
			{
				if (!CGlobalVariable::ipfilter->IsFiltered(uhostIPResult)) {
					bool bVerified = false;
					...
					AddDebugLogLine(false, _T("Ignored kad contact (IP=%s) - IP filter (%s)") , ipstr(uhostIPResult), CGlobalVariable::ipfilter->GetLastHit());
			}
			else if (::thePrefs.GetLogFilteredIPs())
...
void CKademliaUDPListener::Process_KADEMLIA2_RES (const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, CKadUDPKey /*senderUDPKey*/)
{
	...
	try
	{
		for (uint8 iIndex=0; iIndex<uNumContacts; iIndex++)
		{
			...
			{
				if (!CGlobalVariable::ipfilter->IsFiltered(uhostIPResult) && !(uUDPPortResult == 53 && uVersion <= KADEMLIA_VERSION5_48a)  /*No DNS Port without encryption*/) {
					...
					AddDebugLogLine(false, _T("Ignored kad contact (IP=%s:%u) - IP filter (%s)") , ipstr(uhostIPResult), uUDPPortResult,CGlobalVariable::ipfilter->GetLastHit());
				else if (::thePrefs.GetLogFilteredIPs())
...
void CKademliaUDPListener::Process_KADEMLIA_SEARCH_RES (const byte *pbyPacketData, uint32 uLenPacket)
{
	...
	//output the source info to UI eventlist
	CSearch *pSearch = NULL;
	SearchMap::const_iterator itSearchMap = CSearchManager::GetMapSearches().find(uTarget);
	if (itSearchMap != CSearchManager::GetMapSearches().end())
		pSearch = itSearchMap->second;
	if( pSearch && pSearch->GetSearchTypes()==CSearch::FILE && CGlobalVariable::downloadqueue )
	{
		CPartFile* temp = CGlobalVariable::downloadqueue->GetFileByKadFileSearchID(pSearch->GetSearchID());
		if(temp)
		{
			CString sTemp;
			sTemp.Format(GetResString(IDS_KAD_NET),uCount);
			temp->AddFileLog(new CTraceKadMessage(sTemp));
		}
	}
...
void CKademliaUDPListener::Process_KADEMLIA_FIREWALLED_REQ (const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, CKadUDPKey senderUDPKey)
{
	...
	if (!CGlobalVariable::clientlist->RequestTCP(&contact, 0))
...
void CKademliaUDPListener::Process_KADEMLIA_FIREWALLED2_REQ (const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, CKadUDPKey senderUDPKey)
{
	...
	if (!CGlobalVariable::clientlist->RequestTCP(&contact, byConnectOptions))
		return; // cancelled for some reason, don't send a response
...
void CKademliaUDPListener::Process_KADEMLIA_FIREWALLED_RES (const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, CKadUDPKey /*senderUDPKey*/)
{
	...
	else if (!CGlobalVariable::clientlist->IsKadFirewallCheckIP(ntohl(uIP))){ /*KADEMLIA_FIREWALLED2_REQ + KADEMLIA_FIREWALLED_REQ*/
		CString strError;
...
void CKademliaUDPListener::Process_KADEMLIA_FINDBUDDY_REQ (const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, CKadUDPKey senderUDPKey)
{
	...
	if( CKademlia::GetPrefs()->GetFirewalled() || CUDPFirewallTester::IsFirewalledUDP(true) || !CUDPFirewallTester::IsVerified())
		...
		return;
	else if (CGlobalVariable::clientlist->GetBuddyStatus() == Connected)
	...
	contact.SetUDPPort(uUDPPort);
	contact.SetClientID(userID);
	if (!CGlobalVariable::clientlist->IncomingBuddy(&contact, &BuddyID))
		 return; // cancelled for some reason, don't send a response
...
void CKademliaUDPListener::Process_KADEMLIA_FINDBUDDY_RES (const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, CKadUDPKey /*senderUDPKey*/)
{
	...
	if(CKademlia::GetPrefs()->GetKadID() == uCheck)
	{
		...
		CGlobalVariable::clientlist->RequestBuddy(&contact, byConnectOptions);
	}
}
...
void CKademliaUDPListener::Process_KADEMLIA_CALLBACK_REQ (const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, CKadUDPKey /*senderUDPKey*/)
{
	...
	CUpDownClient* pBuddy = CGlobalVariable::clientlist->GetBuddy();
	if( pBuddy != NULL )
	{
		CSafeMemFile fileIO(pbyPacketData, uLenPacket);
		CUInt128 uCheck;
...
void CKademliaUDPListener::SendPacket(const byte *pbyData, uint32 uLenData, uint32 uDestinationHost, uint16 uDestinationPort, CKadUDPKey targetUDPKey, const CUInt128* uCryptTargetID)
{
	...
	CGlobalVariable::clientudp->SendPacket(pPacket, ntohl(uDestinationHost), uDestinationPort, true
		, (uCryptTargetID != NULL) ? uCryptTargetID->GetData() : NULL
		, true , targetUDPKey.GetKeyValue(CGlobalVariable::GetPublicIP(false)));
}
