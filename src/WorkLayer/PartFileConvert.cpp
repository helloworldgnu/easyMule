/* 
 * $Id: PartFileConvert.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
UINT AFX_CDECL CPartFileConvert::run(LPVOID /*lpParam*/)
{
	...
	if (imported)
		CGlobalVariable::sharedfiles->PublishNextTurn();
...
int CPartFileConvert::performConvertToeMule(CString folder)
{
	...
	if (CGlobalVariable::downloadqueue->GetFileByID(file->GetFileHash())!=0) {
		delete file;
	...
	CGlobalVariable::downloadqueue->AddDownload(file,thePrefs.AddNewFilesPaused());
	file->SavePartFile();
	
	if (file->GetStatus(true) == PS_READY)
		CGlobalVariable::sharedfiles->SafeAddKFile(file); // part files are always shared files
