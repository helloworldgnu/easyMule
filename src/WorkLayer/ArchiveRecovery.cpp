/* 
 * $Id: ArchiveRecovery.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
UINT AFX_CDECL CArchiveRecovery::run(LPVOID lpParam)
{
	...
	if (!performRecovery(tp->partFile, tp->filled, tp->preview, tp->bCreatePartFileCopy))
		CGlobalVariable::QueueLogLine(true, _T("%s \"%s\""), GetResString(IDS_RECOVERY_FAILED), tp->partFile->GetFileName());
