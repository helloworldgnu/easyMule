/* 
 * $Id: UDPSocket.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
void CUDPSocket::OnReceive(int nErrorCode)
{
	...
	if (length != SOCKET_ERROR)
	{
		...
		CServer* pServer = CGlobalVariable::serverlist->GetServerByIPUDP(sockAddr.sin_addr.S_un.S_addr, ntohs(sockAddr.sin_port), true);
...
bool CUDPSocket::ProcessPacket(const BYTE* packet, UINT size, UINT opcode, uint32 nIP, uint16 nUDPPort)
{
	try
	{
		...
		CServer* pServer = CGlobalVariable::serverlist->GetServerByIPUDP(nIP, nUDPPort, true);
		if (pServer) {
		...
		switch (opcode)
		{
			case OP_GLOBSEARCHRES:{
				...
				do{
					...
					//  Comment UI
					/*UINT uResultCount = theApp.searchlist->ProcessUDPSearchAnswer(data, true, nIP, nUDPPort-4);
					theApp.emuledlg->searchwnd->AddGlobalEd2kSearchResults(uResultCount); */
					
					UINT uResultCount = CGlobalVariable::searchlist->ProcessUDPSearchAnswer(data, true, nIP, nUDPPort-4);
					SendMessage(CGlobalVariable::m_hListenWnd, WM_SEARCH_ADDED2KRESULT, 0, LPARAM(uResultCount));
 			...
 			case OP_GLOBSERVSTATRES:{
				...
				if (pServer){
					...
					if (pServer->GetLastDescPingedCount() < 2)
					{
						...
						CGlobalVariable::serverconnect->SendUDPPacket(packet, pServer, true);
					}
					else
					{
...
void CUDPSocket::ProcessPacketError(UINT size, UINT opcode, uint32 nIP, uint16 nUDPPort, LPCTSTR pszError)
{
	...
	{
		CString strName;
		CServer* pServer = CGlobalVariable::serverlist->GetServerByIPUDP(nIP, nUDPPort);
		if (pServer)
...
void CUDPSocket::DnsLookupDone(WPARAM wp, LPARAM lp)
{
	...
	if (nIP != INADDR_NONE)
	{
		...
		//  Comment UI
		/*if (!bRemoveServer && CGlobalVariable::ipfilter->IsFiltered(nIP)) {
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("Filtered server \"%s\" (IP=%s) - IP filter (%s)"), pDNSReq->m_pServer->GetAddress(), ipstr(nIP), CGlobalVariable::ipfilter->GetLastHit());
			bRemoveServer = true;
		}*/

		CServer* pServer = CGlobalVariable::serverlist->GetServerByAddress(pDNSReq->m_pServer->GetAddress(), pDNSReq->m_pServer->GetPort());
		if (pServer) {
			pServer->SetIP(nIP);
