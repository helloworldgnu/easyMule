/* 
 * $Id: ClientList.cpp 11398 2009-03-17 11:00:27Z huby $
 * 
...
void CClientList::ResetIP2Country(){
	...
	for(POSITION pos = list.GetHeadPosition(); pos != NULL; list.GetNext(pos)) { 
		cur_client = CGlobalVariable::clientlist->list.GetAt(pos); 
...
#endif
#endif
		CGlobalVariable::uploadqueue->DeleteAll();
		CGlobalVariable::downloadqueue->DeleteAll();
		POSITION pos1, pos2;
	...
	if (found_client != NULL){
		...
		if (sender){
			if (found_client->socket){
				...
				ASSERT( found_client->socket->client==found_client );
...
void CClientList::Process()
{
	...
	for (pos1 = m_KadList.GetHeadPosition(); (pos2 = pos1) != NULL; )
	{
		...
		CEd2kUpDownClient* cur_client =	DYNAMIC_DOWNCAST(CEd2kUpDownClient,m_KadList.GetAt(pos2));
		...
		switch(cur_client->GetKadState())
		{
			...
			case KS_CONNECTED_BUDDY:
				...
				if( m_pBuddy == cur_client && CGlobalVariable::IsFirewalled() && cur_client->SendBuddyPingPong() )
				{
					if (thePrefs.GetDebugClientTCPLevel() > 0)
	...
	if( buddy == Disconnected )
	{
		...
		{
			//  Comment UI
			if( Kademlia::CKademlia::IsRunning() && CGlobalVariable::IsFirewalled() && Kademlia::CUDPFirewallTester::IsFirewalledUDP(true) )
			{
...
#ifdef _DEBUG
void CClientList::Debug_SocketDeleted(CClientReqSocket* deleted,CUpDownClient* /*pOldClient*/,CClientReqSocket* /*pOldReqSocket*/) const
{
	for (POSITION pos = list.GetHeadPosition(); pos != NULL;){
		CUpDownClient* cur_client =	list.GetNext(pos);
...
bool CClientList::RequestTCP(Kademlia::CContact* contact, uint8 byConnectOptions)
{
	...
	if (CGlobalVariable::serverconnect->GetLocalIP() == nContactIP && thePrefs.GetPort() == contact->GetTCPPort())
		return false;

	CUpDownClient* pNewClient = FindClientByIP(nContactIP, contact->GetTCPPort());

	if (!pNewClient)
		pNewClient = new CEd2kUpDownClient(0, contact->GetTCPPort(), contact->GetIPAddress(), 0, 0, false );
...
bool CClientList::IncomingBuddy(Kademlia::CContact* contact, Kademlia::CUInt128* buddyID )
{
	...
	else if (CGlobalVariable::serverconnect->GetLocalIP() == nContactIP && thePrefs.GetPort() == contact->GetTCPPort())
		return false; // don't connect ourself

	//Add client to the lists to be processed.
	CUpDownClient* pNewClient = new CEd2kUpDownClient(0, contact->GetTCPPort(), contact->GetIPAddress(), 0, 0, false );
	pNewClient->SetKadPort(contact->GetUDPPort());
...
/*bool CClientList::DebugDoRequestFirewallCheckUDP(uint32 ip, uint16 port){
	...
	// again mean that the whole check won't work on "Require Obfuscation" setting, which is not a huge problem,
	// but certainly not nice. Only somewhat acceptable way to solve this is to use the KadID instead.
	CUpDownClient* pNewClient = new CEd2kUpDownClient(0, port, ip, 0, 0, false );
	pNewClient->SetKadState(KS_QUEUED_FWCHECK_UDP);
...
void CClientList::CleanUpClientList(){
	...
	if (m_dwLastClientCleanUp + CLIENTLIST_CLEANUP_TIME < cur_tick ){
		...
		for (pos1 = list.GetHeadPosition();( pos2 = pos1 ) != NULL;)
		{
			...
			if( pCurClient->IsKindOf(RUNTIME_CLASS(CHttpClient)) 
				&& pCurClient->GetDownloadState()==DS_REDIRECTED)
			{
				cDeleted++;
				delete pCurClient;							
				continue;
