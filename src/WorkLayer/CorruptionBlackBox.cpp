/* 
 * $Id: CorruptionBlackBox.cpp 5726 2008-06-03 08:00:08Z huby $
 * 
...
CCBBRecord::CCBBRecord(uint64 nStartPos, uint64 nEndPos, uint32 dwIP, DWORD dwIPFrom, EBBRStatus BBRStatus){
...
CCBBRecord& CCBBRecord::operator=(const CCBBRecord& cv)
{
	...
	m_dwFrom = cv.m_dwFrom;

	return *this; 
}

bool CCBBRecord::Merge(uint64 nStartPos, uint64 nEndPos, uint32 dwIP, DWORD dwFrom, EBBRStatus BBRStatus){

	if (m_dwIP == dwIP && m_dwFrom == dwFrom && m_BBRStatus == BBRStatus && (nStartPos == m_nEndPos + 1 || nEndPos + 1 == m_nStartPos)){
...
bool CCBBRecord::CanMerge(uint64 nStartPos, uint64 nEndPos, uint32 dwIP, DWORD dwFrom, EBBRStatus BBRStatus){
...
void CCorruptionBlackBox::TransferredData(uint64 nStartPos, uint64 nEndPos, const CUpDownClient* pSender){
	...
	DWORD  dwSenderIPFrom = pSender->GetIPFrom( );
	...
	for (int i= 0; i < m_aaRecords[nPart].GetCount(); i++){
		if (m_aaRecords[nPart][i].CanMerge(nRelStartPos, nRelEndPos, dwSenderIP,dwSenderIPFrom, BBR_NONE)){
			posMerge = i;
		}
		// check if there is already an pending entry and overwrite it
			...
			if (m_aaRecords[nPart][i].m_nStartPos >= nRelStartPos && m_aaRecords[nPart][i].m_nEndPos <= nRelEndPos){
				...
				if (dwSenderIP != m_aaRecords[nPart][i].m_dwIP){
					...
					DWORD  dwOldIPFrom = m_aaRecords[nPart][i].m_dwFrom;
					m_aaRecords[nPart][i].m_dwIP = dwSenderIP;
					m_aaRecords[nPart][i].m_dwFrom = dwSenderIPFrom;
					m_aaRecords[nPart].Add(CCBBRecord(nTmpStartPos1,nTmpEndPos1, dwOldIP, dwOldIPFrom));
					m_aaRecords[nPart].Add(CCBBRecord(nTmpStartPos2,nTmpEndPos2, dwOldIP, dwOldIPFrom));
					// and are done then
				}
	...
	if (posMerge != (-1) ){
		VERIFY( m_aaRecords[nPart][posMerge].Merge(nRelStartPos, nRelEndPos, dwSenderIP,dwSenderIPFrom, BBR_NONE) );
	}
	else
		m_aaRecords[nPart].Add(CCBBRecord(nRelStartPos, nRelEndPos, dwSenderIP,dwSenderIPFrom, BBR_NONE));
	...
	for (int i= 0; i < m_aaRecords[nPart].GetCount(); i++){
		if (m_aaRecords[nPart][i].m_BBRStatus == BBR_NONE || m_aaRecords[nPart][i].m_BBRStatus == BBR_VERIFIED){
			if (m_aaRecords[nPart][i].m_nStartPos >= nRelStartPos && m_aaRecords[nPart][i].m_nEndPos <= nRelEndPos){
				...
				m_aaRecords[nPart][i].m_nEndPos = nRelEndPos;
				m_aaRecords[nPart][i].m_nStartPos = nRelStartPos;
				m_aaRecords[nPart].Add(CCBBRecord(nTmpStartPos1, nTmpEndPos1, m_aaRecords[nPart][i].m_dwIP, m_aaRecords[nPart][i].m_dwFrom, m_aaRecords[nPart][i].m_BBRStatus));
				m_aaRecords[nPart].Add(CCBBRecord(nTmpStartPos2, nTmpEndPos2, m_aaRecords[nPart][i].m_dwIP, m_aaRecords[nPart][i].m_dwFrom, m_aaRecords[nPart][i].m_BBRStatus));
				nDbgVerifiedBytes +=  (m_aaRecords[nPart][i].m_nEndPos-m_aaRecords[nPart][i].m_nStartPos)+1;
				m_aaRecords[nPart][i].m_BBRStatus = BBR_VERIFIED;
				DEBUG_ONLY(mapDebug.SetAt(m_aaRecords[nPart][i].m_dwIP, 1));
			}
				...
				m_aaRecords[nPart][i].m_nEndPos = nRelEndPos;
				m_aaRecords[nPart].Add(CCBBRecord(nTmpStartPos, nTmpEndPos, m_aaRecords[nPart][i].m_dwIP, m_aaRecords[nPart][i].m_dwFrom, m_aaRecords[nPart][i].m_BBRStatus));
				nDbgVerifiedBytes +=  (m_aaRecords[nPart][i].m_nEndPos-m_aaRecords[nPart][i].m_nStartPos)+1;
				m_aaRecords[nPart][i].m_BBRStatus = BBR_VERIFIED;
				DEBUG_ONLY(mapDebug.SetAt(m_aaRecords[nPart][i].m_dwIP, 1));
			}
...
void CCorruptionBlackBox::CorruptedData(uint64 nStartPos, uint64 nEndPos){
	...
	for(int k = 0; k < aGuiltyClients.GetCount();){
		...
		if (CGlobalVariable::clientlist->IsBannedClient(aGuiltyClients[k])){
	...
	if (aGuiltyClients.GetCount() > 0){
		...
		for(int k = 0; k < aGuiltyClients.GetCount(); k++){
			...
			if ( nCorruptPercentage > CBB_BANTHRESHOLD){
				...
				CUpDownClient* pEvilClient = CGlobalVariable::clientlist->FindClientByIP(aGuiltyClients[k]);
				if (pEvilClient != NULL){
					...
					CGlobalVariable::clientlist->AddTrackClient(pEvilClient);
					...
					CGlobalVariable::clientlist->AddBannedClient(aGuiltyClients[k]);
				}
			}
			else{
				CUpDownClient* pSuspectClient = CGlobalVariable::clientlist->FindClientByIP(aGuiltyClients[k]);
				if (pSuspectClient != NULL){
					...
					CGlobalVariable::clientlist->AddTrackClient(pSuspectClient);
				}
				else
...
uint32  CCorruptionBlackBox::TotalDataFromIP( uint32 dwIP,uint32 nPart )
{
	uint32 iTotalData = 0;

	for (int i= 0; i < m_aaRecords[nPart].GetCount(); i++)
	{
		if( m_aaRecords[nPart][i].m_dwIP==dwIP )
			iTotalData += uint32(m_aaRecords[nPart][i].m_nEndPos - m_aaRecords[nPart][i].m_nStartPos + 1);
	}

	return iTotalData;
}

uint32  CCorruptionBlackBox::TotalDataFromSiteType( DWORD dwIPFrom,uint32 nPart )
{
	uint32 iTotalData = 0;

	for (int i= 0; i < m_aaRecords[nPart].GetCount(); i++)
	{
		if( m_aaRecords[nPart][i].m_dwFrom == dwIPFrom )
			iTotalData += uint32(m_aaRecords[nPart][i].m_nEndPos - m_aaRecords[nPart][i].m_nStartPos + 1);
	}
...
uint32  CCorruptionBlackBox::TotalDataFromUrlSite( uint32 nPart )
{
	uint32 iTotalData = 0;

	for (int i= 0; i < m_aaRecords[nPart].GetCount(); i++)
	{
		if( m_aaRecords[nPart][i].m_dwFrom >= sfUnknwon )
			iTotalData += uint32(m_aaRecords[nPart][i].m_nEndPos - m_aaRecords[nPart][i].m_nStartPos + 1);
	}
