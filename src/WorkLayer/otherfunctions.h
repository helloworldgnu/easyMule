/* 
 * $Id: otherfunctions.h 9780 2009-01-07 07:58:37Z dgkang $
 * 
...
bool SelectDir(HWND hWnd, LPTSTR pszPath, LPCTSTR pszTitle = NULL, LPCTSTR pszDlgTitle = NULL, BOOL bHasNewFolderBn = TRUE);
...
BOOL TestUrl(LPCTSTR pszURL, BOOL bTestExt = TRUE);
...
inline int GetXunLeiVer(CString ModVer)
{
	int ver = 0;

	if (ModVer.GetLength() > 8)
	{
		if ( ModVer.Left(8) == _T("xl build"))
		{
			for(int i = 8; i < ModVer.GetLength(); i++)
			{
				if (!isdigit(ModVer[i]))
					return 0;
			}
			ver = _tstoi((LPCTSTR)ModVer.Mid(8));
			return ver;
		}
	}
	return 0;
