/* 
 * $Id: FirewallOpener.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
bool CFirewallOpener::AddRule(const CICSRuleInfo& riPortRule, const INetSharingConfigurationPtr pNSC, const INetConnectionPropsPtr pNCP){
		...
		if ( SUCCEEDED(hr) && SUCCEEDED(pNSPM->Enable())){
			...
			// IFWS END
			CGlobalVariable::QueueDebugLogLine(false, _T("Succeeded to add Rule '%s' for Port '%u' on Connection '%s'"),riPortRule.m_strRuleName, riPortRule.m_nPortNumber, CString(bstrName));
			return true;
		}
		else{
			CGlobalVariable::QueueDebugLogLine(false, _T("Failed to add Rule '%s' for Port '%u' on Connection '%s'"),riPortRule.m_strRuleName, riPortRule.m_nPortNumber, CString(bstrName));
			return false;
		}
}
			...
			switch(eAction){
				...
				case FOC_DELETERULEEXCACT:
					...
					//	&& riPortRule.m_nPortNumber == uExternal && riPortRule.m_byProtocol == ucProt)
					{
						...
						CGlobalVariable::QueueDebugLogLine(false,_T("Rule removed"));
					}
					break;
				case FOC_DELETERULEBYNAME:
					if (riPortRule.m_strRuleName == CString(bstrName) && bstrTargetIP == CComBSTR("127.0.0.1")){ // NEO: IFWS - [ICSFirewall] <-- Xanatos --
