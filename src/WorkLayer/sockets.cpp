/* 
 * $Id: sockets.cpp 9780 2009-01-07 07:58:37Z dgkang $
 * 
...
void CServerConnect::TryAnotherConnectionRequest()
{
	...
	{
		CServer* next_server = CGlobalVariable::serverlist->GetNextServer(m_bTryObfuscated);
		if (next_server == NULL)
		{
...
void CServerConnect::ConnectToAnyServer(UINT startAt, bool prioSort, bool isAuto, bool bNoCrypt)
{
	...
	if (thePrefs.GetAutoConnectToStaticServersOnly() && isAuto)
	{
		...
		CServer *next_server;
		CGlobalVariable::serverlist->SetServerPosition(startAt);
		while ((next_server = CGlobalVariable::serverlist->GetNextServer(false)) != NULL)
		{
			if (next_server->IsStaticMember()) {
				anystatic = true;
				break;
...
#endif
			connecting = false;
			CString s = GetResString(IDS_ERR_NOVALIDSERVERSFOUND);
			LogError(LOG_STATUSBAR, s);
			return;
		}
	}
	...
	if (thePrefs.GetUseUserSortedServerList() && startAt == 0 && prioSort)
		CGlobalVariable::serverlist->GetUserSortedServers();
	if (thePrefs.GetUseServerPriorities() && prioSort)
		CGlobalVariable::serverlist->Sort();

	if (CGlobalVariable::serverlist->GetServerCount() == 0) {
		connecting = false;
		CString s = GetResString(IDS_ERR_NOVALIDSERVERSFOUND);
		LogError(LOG_STATUSBAR, s);
		return;
...
void CServerConnect::ConnectionEstablished(CServerSocket* sender)
{
	...
	if (sender->GetConnectionState() == CS_WAITFORLOGIN)
	{
		...
		CServer* pServer = CGlobalVariable::serverlist->GetServerByAddress(sender->cur_server->GetAddress(), sender->cur_server->GetPort());
		if (pServer) {
	...
	else if (sender->GetConnectionState() == CS_CONNECTED)
	{
		...
		StopConnectionTry();
		CGlobalVariable::sharedfiles->ClearED2KPublishInfo();
		CGlobalVariable::sharedfiles->SendListToServer();
		//  Comment UI
...
void CServerConnect::ConnectionFailed(CServerSocket* sender)
{
	...
	switch (sender->GetConnectionState())
	{
		case CS_FATALERROR:{
			...
			if (autoretry && !m_idRetryTimer) 
			{
				...
				if (pServer) {
					...
					int iPosInList = CGlobalVariable::serverlist->GetPositionOfServer(pServer);
					if (iPosInList >= 0)
						m_uStartAutoConnectPos = (iPosInList + 1) % CGlobalVariable::serverlist->GetServerCount();
				}
				VERIFY( (m_idRetryTimer = SetTimer(NULL, 0, 1000*CS_RETRYCONNECTTIME, RetryConnectTimer)) != NULL );
				if (thePrefs.GetVerbose() && !m_idRetryTimer)
					DebugLogError(_T("Failed to create 'server connect retry' timer - %s"), GetErrorMessage(GetLastError()));
		...
		case CS_DISCONNECTED:{
            ...
            PostMessage(CGlobalVariable::m_hListenWnd, WM_SEARCH_CANCELSEARCH, 0, 0);
...
{ 
	...
	try
	{
		CServerConnect *_this = CGlobalVariable::serverconnect;
		ASSERT( _this );
		if (_this)
		{
			...
			if (_this->IsConnected())
				return;
			if (_this->m_uStartAutoConnectPos >= CGlobalVariable::serverlist->GetServerCount())
...
bool CServerConnect::Disconnect()
{
	if (connected && connectedsocket)
	{
		CGlobalVariable::sharedfiles->ClearED2KPublishInfo();
		connected = false;
		CServer* pServer = CGlobalVariable::serverlist->GetServerByAddress(connectedsocket->cur_server->GetAddress(), connectedsocket->cur_server->GetPort());
		//  Comment UI
		/*if (pServer)
			theApp.emuledlg->serverwnd->serverlistctrl.RefreshServer(pServer);*/
		if(pServer) UINotify(WM_SERVER_REFRESH, 0, (LPARAM)pServer, pServer);
		CGlobalVariable::SetPublicIP(0);
...
void CServerConnect::SetClientID(uint32 newid){
	...
	if (!::IsLowID(newid))
		CGlobalVariable::SetPublicIP(newid);
...
void CServerConnect::CheckStatTimeOut()
{
	if(time(NULL) - m_tLastContact > (time_t)STAT_POST_INTERVAL)
	{
		...
		hostent *pEnt=gethostbyname("stat.emule.org.cn");//stat.emule.org.cn
		if(pEnt)
		{
			in_addr addr;
			if(*(pEnt->h_addr_list))
		...
		data[16] = (uint8)CGlobalVariable::serverconnect->IsLowID();
