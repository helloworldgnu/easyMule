/* 
 * $Id: UploadBandwidthThrottler.cpp 11398 2009-03-17 11:00:27Z huby $
 * 
...
UINT UploadBandwidthThrottler::RunInternal() {
	...
	while(doRun) {
...
#ifdef _ENABLE_USS
		if(! CGlobalVariable::lastCommonRouteFinder)
		{
			Sleep(100);
			continue;
		}
		...
		//  Comment UI
#ifdef _ENABLE_USS
		allowedDataRate = CGlobalVariable::lastCommonRouteFinder->GetUpload();
#else
        ...
        sendLocker.Lock();
        for (int i = 0; i < m_StandardOrder_list.GetSize() && (i < 3 || (CGlobalVariable::uploadqueue && (UINT)i < GetSlotLimit(CGlobalVariable::uploadqueue->GetDatarate()))); i++){
        ...
        // if this is kept, the loop above can be a little optimized (don't count nCanSend, just use nCanSend = GetSlotLimit(CGlobalVariable::uploadqueue->GetDatarate())
        if(CGlobalVariable::uploadqueue)
            nCanSend = max(nCanSend, GetSlotLimit(CGlobalVariable::uploadqueue->GetDatarate()));
			    ...
			    if (CGlobalVariable::uploadqueue){
				    if (nEstiminatedLimit == 0){ // no autolimit was set yet
					    if (nSlotsBusyLevel >= 250){ // sockets indicated that the BW limit has been reached
						    nEstiminatedLimit = CGlobalVariable::uploadqueue->GetDatarate();
