/* 
 * $Id: SecRunAsUser.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
eResult CSecRunAsUser::PrepareUser(){
	...
	try{
		...
		try{
			...
			// clean up and abort
			CGlobalVariable::QueueDebugLogLine(false, _T("Run as unpriveleged user: Exception while preparing user account: %s!"), error);
			CoUninitialize();
		...
		// clean up and abort
		CGlobalVariable::QueueDebugLogLine(false, _T("Run as unpriveleged user: Unexpected fatal error while preparing user account!"));
		CoUninitialize();
		return RES_FAILED;
	}
...
#define FULLACCESS ADS_RIGHT_GENERIC_ALL
	...
	if (!bSucceeded)
		CGlobalVariable::QueueDebugLogLine(false, _T("Run as unpriveleged user: Error: Failed to set directoy permissions!"));
...
eResult CSecRunAsUser::RestartAsUser(){
	...
	try{
		...
		CGlobalVariable::QueueDebugLogLine(false, _T("Run as unpriveleged user: Error: Unexpected exception while loading advapi32.dll"));
		FreeAPI();
		return RES_FAILED;
	}
	FreeAPI();
	if (!bResult)
		CGlobalVariable::QueueDebugLogLine(false, _T("Run as unpriveleged user: Error: Failed to restart eMule as different user! Error Code: %i"),GetLastError());
...
eResult CSecRunAsUser::RestartAsRestricted(){
	...
	try{
		...
		CGlobalVariable::QueueDebugLogLine(false, _T("SecureShellExecute exception: %s!"), strError);
...
eResult CSecRunAsUser::RestartSecure(){
	...
	if (!thePrefs.IsPreferingRestrictedOverUser()){
		res = PrepareUser();;
		if (res == RES_OK){
			CGlobalVariable::QueueLogLine(false, GetResString(IDS_RAU_RUNNING), EMULEACCOUNT);
			return RES_OK;
		}
	...
	if (res == RES_OK){
		CGlobalVariable::QueueLogLine(false, GetResString(IDS_RUNNINGRESTRICTED));
	}
