/* 
 * $Id: TransferWnd.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
{
	TCHAR buffer[100];
	_stprintf(buffer,_T("%u (%u ") + GetResString(IDS_BANNED).MakeLower() + _T(")"), number,CGlobalVariable::clientlist->GetBannedCount() );
		...
		case IDC_DOWNLOADLIST + IDC_UPLOADLIST: {
			...
			switch (m_uWnd2)
			{
				case wnd2Uploading: {
					...
					uint32 activeCount = CGlobalVariable::uploadqueue->GetActiveUploadsCount();
...
void CTransferWnd::OnNMRclickDltab(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
	...
	menu.AppendMenu(MF_STRING,MP_CANCEL,GetResString(IDS_POPMENU_DELETE),_T("DELETE") );
...
BOOL CTransferWnd::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	{ 
		...
		case MP_CAT_REMOVE: {
			...
			CGlobalVariable::downloadqueue->ResetCatParts(rightclickindex);
		...
		case MP_PAUSE:
			CGlobalVariable::downloadqueue->SetCatStatus(rightclickindex,MP_PAUSE);
			break;
		case MP_STOP:
			CGlobalVariable::downloadqueue->SetCatStatus(rightclickindex,MP_STOP);
			break;
		case MP_CANCEL:
			 if(MessageBox(GetResString(IDS_Q_CANCELDL),GetResString(IDS_CAPTION),MB_ICONQUESTION|MB_YESNO) == IDYES)
				CGlobalVariable::downloadqueue->SetCatStatus(rightclickindex,MP_CANCEL);
			break;
		case MP_RESUME:
			CGlobalVariable::downloadqueue->SetCatStatus(rightclickindex,MP_RESUME);
			break;
#ifdef _PLAY_WHILE_DOWNLOADING
		case MP_PLAY:
			CGlobalVariable::downloadqueue->SetCatStatus(rightclickindex, MP_PLAY); // VC-SearchDream[2007-05-16]: for see movie while downloading
			break;
#endif 
		case MP_RESUMENEXT:
			CGlobalVariable::downloadqueue->StartNextFile(rightclickindex,false);
			break;
		...
		case MP_DOWNLOAD_ALPHABETICAL: {
            ...
            if(newSetting) {
                ...
                CGlobalVariable::downloadqueue->RemoveAutoPrioInCat(rightclickindex, PR_NORMAL);
            }
...
void CTransferWnd::EditCatTabLabel(int index,CString newlabel)
{
	...
	if (thePrefs.ShowCatTabInfos()) {
		...
		for (int i=0;i<CGlobalVariable::downloadqueue->GetFileCount();i++) {
			cur_file=CGlobalVariable::downloadqueue->GetFileByIndex(i);
...
CString CTransferWnd::GetTabStatistic(int tab)
{
	...
	for (int i = 0; i < CGlobalVariable::downloadqueue->GetFileCount(); i++)
	{
		/*const*/ CPartFile* cur_file = CGlobalVariable::downloadqueue->GetFileByIndex(i);
		if (cur_file == 0)
			continue;
...
void CTransferWnd::OnTabMovement(NMHDR* /*pNMHDR*/, LRESULT* /*pResult*/)
{
	...
	CGlobalVariable::downloadqueue->MoveCat((uint8)from,(uint8)to);
