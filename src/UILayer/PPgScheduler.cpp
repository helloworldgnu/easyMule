/* 
 * $Id: PPgScheduler.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
BOOL CPPgScheduler::OnInitDialog()
{
	...
	m_ttc.Create(this);
	m_ttc.AddTool(GetDlgItem(IDC_NEW), GetResString(IDS_TIP_SCHEDULER_NEW));
	m_ttc.AddTool(GetDlgItem(IDC_REMOVE), GetResString(IDS_TIP_SCHEDULER_REMOVE));
...
void CPPgScheduler::Localize(void)
{
	if(m_hWnd)
	{
		...
		m_ttc.UpdateTipText(GetResString(IDS_TIP_SCHEDULER_NEW), GetDlgItem(IDC_NEW));
		m_ttc.UpdateTipText(GetResString(IDS_TIP_SCHEDULER_REMOVE), GetDlgItem(IDC_REMOVE));
...
BOOL CPPgScheduler::PreTranslateMessage(MSG* pMsg)
{
