/* 
 * $Id: ServerListCtrl.cpp 7701 2008-10-15 07:34:41Z huby $
 * 
...
{
	for (POSITION pos = CGlobalVariable::serverlist->list.GetHeadPosition(); pos != NULL; ) 
	{
		RefreshServer(CGlobalVariable::serverlist->list.GetNext(pos));
	}
}
...
void CServerListCtrl::RemoveServer(const CServer* pServer)
{
	...
	if (iItem != -1) {
		CGlobalVariable::serverlist->RemoveServer(pServer);
		DeleteItem(iItem); 
		ShowServerCount();
	}
}
...
{
	ShowWindow(SW_HIDE);
	for(POSITION pos = CGlobalVariable::serverlist->list.GetHeadPosition(); pos != NULL; CGlobalVariable::serverlist->list.GetNext(pos))
	{
		const CServer* cur_server = CGlobalVariable::serverlist->list.GetAt(pos);
		if (cur_server->GetFailedCount() >= thePrefs.GetDeadServerRetries())
		{
			RemoveServer(cur_server);
			pos = CGlobalVariable::serverlist->list.GetHeadPosition();
		}
	}
	ShowWindow(SW_SHOW);
}
...
void CServerListCtrl::RemoveAllFilteredServers()
{
	...
	{
		const CServer* cur_server = CGlobalVariable::serverlist->list.GetAt(pos);
		if (CGlobalVariable::ipfilter->IsFiltered(cur_server->GetIP()))
		{
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("Filtered server \"%s\" (IP=%s) - IP filter (%s)"), cur_server->GetListName(), ipstr(cur_server->GetIP()), CGlobalVariable::ipfilter->GetLastHit());
			RemoveServer(cur_server);
			pos = CGlobalVariable::serverlist->list.GetHeadPosition();
		}
	}
...
{
	if (!CGlobalVariable::serverlist->AddServer(pServer))
		return false;
	if (bAddToList)
	{
...
{ 
	...
	cif.AddIcon((UINT_PTR)ServerPrioMenu.m_hMenu, theApp.LoadIcon(_T("PRIORITY")));
	cif.AddIcon((UINT_PTR)ServerPrioMenu.m_hMenu, theApp.LoadIcon(_T("PRIORITY")), TRUE);
	...
	m_pMenuXP->AddMenu(&ServerMenu, TRUE);
...
BOOL CServerListCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
		...
		if (GetSelectedCount() > 1)
		{
			...
			while (pos != NULL)
			{
				...
				if (iItem > -1) {
					...
					CGlobalVariable::serverlist->MoveServerDown(pServer);
				}
			}
			CGlobalVariable::serverconnect->ConnectToAnyServer(CGlobalVariable::serverlist->GetServerCount() - GetSelectedCount(), false, false);
		}
		else
		{
			int iItem = GetNextItem(-1, LVIS_SELECTED | LVIS_FOCUSED);
			if (iItem > -1)
				CGlobalVariable::serverconnect->ConnectToServer((CServer*)GetItemData(iItem));
		}
	...
	case MPG_DELETE: {
		...
		while (GetFirstSelectedItemPosition() != NULL)
		{
			POSITION pos = GetFirstSelectedItemPosition();
			int iItem = GetNextSelectedItem(pos);
			CGlobalVariable::serverlist->RemoveServer((CServer*)GetItemData(iItem));
			DeleteItem(iItem);
		}
		ShowServerCount();
		SetRedraw(TRUE);
	...
	case MP_REMOVEALL:
		...
		if (CGlobalVariable::serverconnect->IsConnecting()) {
			CGlobalVariable::downloadqueue->StopUDPRequests();
			CGlobalVariable::serverconnect->StopConnectionTry();
			CGlobalVariable::serverconnect->Disconnect();
			theApp.emuledlg->ShowConnectionState();
		}
		ShowWindow(SW_HIDE);
		CGlobalVariable::serverlist->RemoveAllServers();
		DeleteAllItems();
		ShowWindow(SW_SHOW);
		ShowServerCount();
		return TRUE;
...
void CServerListCtrl::SetSelectedServersPriority(UINT uPriority)
{
	...
	if (bUpdateStaticServersFile)
		CGlobalVariable::serverlist->SaveStaticServers();
}
...
void CServerListCtrl::OnNMDblClk(NMHDR* /*pNMHDR*/, LRESULT* /*pResult*/)
{
	...
	if (iSel != -1)
	{
		CGlobalVariable::serverconnect->ConnectToServer((CServer*)GetItemData(iSel));
	   	theApp.emuledlg->ShowConnectionState();
	}
}
...
{
	SetRedraw(FALSE);
	bool bResult = CGlobalVariable::serverlist->AddServerMetToList(strFile, true);
...
void CServerListCtrl::OnNMCustomDraw(NMHDR *pNMHDR, LRESULT *plResult)
{
	...
	if (pnmlvcd->nmcd.dwDrawStage == CDDS_ITEMPREPAINT)
	{
		...
		const CServer* pConnectedServer = CGlobalVariable::serverconnect->GetCurrentServer();
...
void CServerListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) // 与 IP to Country 有关，不能注释掉
{
	...
	CServer* pServer = (CServer *)lpDrawItemStruct->itemData;
	pServer->ResetIP2Country();
				...
				case 0:
					{
						...
						if (CGlobalVariable::serverconnect->IsConnected()
							&& (cur_srv = CGlobalVariable::serverconnect->GetCurrentServer()) != NULL
					...
					if(CGlobalVariable::ip2country->ShowCountryFlag()){
						POINT point = {cur_rec.left, cur_rec.top};//check
						CGlobalVariable::ip2country->GetFlagImageList()->DrawIndirect(dc, server->GetCountryFlagIndex(), point, CSize(18,16), CPoint(0,0), ILD_NORMAL);
						cur_rec.left +=20;
					}
					//EastShare End - added by AndCycle, IP to Country
					break;
...
void CServerListCtrl::OnDrawItem(int /*nIDCtl*/, LPDRAWITEMSTRUCT lpDrawItemStruct)// VC-linhai[2007-08-07]:warning C4100: “nIDCtl” : 未引用的形参
{
	m_pMenuXP->DrawItem(lpDrawItemStruct);
}

void CServerListCtrl::OnMeasureItem(int /*nIDCtl*/, LPMEASUREITEMSTRUCT lpMeasureItemStruct)// VC-linhai[2007-08-07]:warning C4100: “nIDCtl” : 未引用的形参
{
