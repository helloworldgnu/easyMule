/* 
 * $Id: SearchListCtrl.cpp 9297 2008-12-24 09:55:04Z dgkang $
 * 
...
void CSearchListCtrl::UpdateSources(const CSearchFile* toupdate)
{
	...
	if (index != -1)
	{
		...
		{
			const SearchList* list = CGlobalVariable::searchlist->GetSearchListForID(toupdate->GetSearchID());
	...
	while (pos != NULL)
	{
		...
		if (pFile)
		{
			...
			if (pFile->IsPreviewPossible())
				iToPreview++;
			if (!CGlobalVariable::downloadqueue->IsFileExisting(pFile->GetFileHash(), false))
				iToDownload++;
		}
	}
		...
		switch (wParam)
		{
			...
			case MPG_DELETE:
			{
				...
				while (pos != NULL)
				{
					file = selectedList.GetNext(pos);
					HideSources(file);
					CGlobalVariable::searchlist->RemoveResult(file);
				}
				AutoSelectItem();
				SetRedraw(TRUE);
				return TRUE;
			...
			case MP_PREVIEW:
				if (file){
					if (file->GetPreviews().GetSize() > 0){
						...
						CUpDownClient* newclient = new CEd2kUpDownClient(NULL, file->GetClientPort(),file->GetClientID(),file->GetClientServerIP(),file->GetClientServerPort(), true);
						if (!CGlobalVariable::clientlist->AttachToAlreadyKnown(&newclient,NULL)){
							CGlobalVariable::clientlist->AddClient(newclient);
						}

						CEd2kUpDownClient * ed2k_client = STATIC_DOWNCAST( CEd2kUpDownClient , newclient );
						if( ed2k_client )
							ed2k_client->SendPreviewRequest(file);
...
void CSearchListCtrl::ExpandCollapseItem(int iItem, int iAction)
{
	...
	if (!searchfile->IsListExpanded())
	{
		if (iAction > COLLAPSE_ONLY)
		{
			...
			SetRedraw(FALSE);
			const SearchList* list = CGlobalVariable::searchlist->GetSearchListForID(searchfile->GetSearchID());
			for (POSITION pos = list->GetHeadPosition(); pos != NULL; )
			{
				const CSearchFile* cur_file = list->GetNext(pos);
...
{
	const CKnownFile* pFile = CGlobalVariable::downloadqueue->GetFileByID(src->GetFileHash());
	...
	else if (CGlobalVariable::sharedfiles->GetFileByID(src->GetFileHash()))
	{
		src->SetKnownType(CSearchFile::Shared);
		return m_crSearchResultShareing;
	}
	else if (CGlobalVariable::knownfiles->FindKnownFileByID(src->GetFileHash()))
	{
		src->SetKnownType(CSearchFile::Downloaded);
		return m_crSearchResultKnown;
	}
	else if (CGlobalVariable::knownfiles->IsCancelledFileByID(src->GetFileHash())){
...
void CSearchListCtrl::RemoveResults(uint32 nID)
{
	searchlist->RemoveResults(nID);
}
