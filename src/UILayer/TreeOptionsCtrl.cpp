/*
 * $Id: TreeOptionsCtrl.cpp 5134 2008-03-26 04:07:04Z wangna $
 * 
...
void CTreeOptionsCtrl::OnKillFocus(CWnd* pNewWnd) 
{
	...
	if (bForeignWnd)
	{
		CTreeOptionsCtrl::UpdateTreeControlValueFromChildControl(GetSelectedItem());
		DestroyOldChildControl();
	}
...
void CTreeOptionsCtrl::HandleChildControlLosingFocus(bool bLosingFocus)
{
	//Clean up any controls currently open we used
...
void CTreeOptionsEdit::OnKillFocus(CWnd* pNewWnd) 
{
	...
	{
		ASSERT(m_pTreeCtrl);
		if (m_pTreeCtrl->m_hOldItem == m_pTreeCtrl->GetSelectedItem() || m_pTreeCtrl->m_hControlItem == m_pTreeCtrl->GetSelectedItem())
		{
		   m_pTreeCtrl->HandleChildControlLosingFocus(true);
		}
	}
}
...
void DDX_TreeSetEdit(CDataExchange* pDX, int nIDC, HTREEITEM hItem, CString&  strValue)
{
	HWND hWndCtrl = pDX->PrepareCtrl(nIDC);
	CTreeOptionsCtrl* pCtrlTreeOptions = (CTreeOptionsCtrl*) CWnd::FromHandlePermanent(hWndCtrl);
	ASSERT(pCtrlTreeOptions);
	...
	pCtrlTreeOptions->SetEditText(hItem,strValue);
}
