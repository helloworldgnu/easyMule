/*
 * $Id: MuleSystrayDlg.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
BOOL CMuleSystrayDlg::OnInitDialog() 
{
	...
	p = GetDlgItem(IDC_UPDATE);
	if(p)
	{
		p->GetWindowRect(r);
		ScreenToClient(r);
		m_ctrlUpdate.Create(NULL, NULL, WS_CHILD|WS_VISIBLE, r, this, IDC_UPDATE);
		m_ctrlUpdate.m_nBtnID = IDC_UPDATE;
		m_ctrlUpdate.m_strText = GetResString(IDS_VERSIONCHECK);
		m_ctrlUpdate.m_strText.Remove(_T('&'));

		m_ctrlUpdate.m_bUseIcon = true;
		m_ctrlUpdate.m_sIcon.cx = 16;
		m_ctrlUpdate.m_sIcon.cy = 16;
		m_ctrlUpdate.m_hIcon = theApp.LoadIcon(_T("CheckNewVersion"), m_ctrlUpdate.m_sIcon.cx, m_ctrlUpdate.m_sIcon.cy);
		m_ctrlUpdate.m_bParentCapture = true;
		if(bValidFont)
		{	
			m_ctrlUpdate.m_cfFont.CreateFontIndirect(&lfStaticFont);
		}	
	}
	...
	m_ctrlSidebar.SetWindowText(_T("easyMule ") + CGlobalVariable::GetCurVersionLong());
