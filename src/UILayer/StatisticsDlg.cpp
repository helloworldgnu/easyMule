/* 
 * $Id: StatisticsDlg.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
void CStatisticsDlg::SetCurrentRate(float uploadrate, float downloadrate)
{
	...
	m_dPlotDataUp[4] = uploadrate - (float)CGlobalVariable::uploadqueue->GetToNetworkDatarate() / 1024;
	m_UploadOMeter.AppendPoints(m_dPlotDataUp);

	// Connections
	CDownloadQueue::SDownloadStats myStats;
	CGlobalVariable::downloadqueue->GetDownloadStats(myStats);
	m_dPlotDataMore[0] = CGlobalVariable::listensocket->GetActiveConnections();
	m_dPlotDataMore[1] = CGlobalVariable::uploadqueue->GetActiveUploadsCount();
	m_dPlotDataMore[2] = CGlobalVariable::uploadqueue->GetUploadQueueLength();
	...
	updown.connections = CGlobalVariable::listensocket->GetActiveConnections();
	CGlobalVariable::webserver->AddStatsLine(updown);
}
...
void CStatisticsDlg::ShowStatistics(bool forceUpdate) 
{
	...
	if (forceUpdate || stattree.IsExpanded(h_transfer)) 
	{
		...
		if (forceUpdate || stattree.IsExpanded(h_download)) 
		{
			...
			if (forceUpdate || stattree.IsExpanded(h_down_session)) 
			{
				...
				{ 
					...
					cbuffer.Format(_T("%s: %s, %s: %s (%.1f%%)"), GetResString(IDS_UDPREASKS), CastItoIShort(CGlobalVariable::downloadqueue->GetUDPFileReasks()), GetResString(IDS_UFAILED), CastItoIShort(CGlobalVariable::downloadqueue->GetFailedUDPFileReasks()), CGlobalVariable::downloadqueue->GetUDPFileReasks() ? (CGlobalVariable::downloadqueue->GetFailedUDPFileReasks() * 100.0 / CGlobalVariable::downloadqueue->GetUDPFileReasks()) : 0.0 );
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;

					cbuffer.Format(_T("%s: %s (%s + %s)"), GetResString(IDS_DEADSOURCES), CastItoIShort(CGlobalVariable::clientlist->m_globDeadSourceList.GetDeadSourcesCount() + myStats.a[22]), CastItoIShort(CGlobalVariable::clientlist->m_globDeadSourceList.GetDeadSourcesCount()), CastItoIShort((UINT)myStats.a[22]));
					stattree.SetItemText( down_sources[i] , cbuffer );
					i++;
				}
		...
		if (forceUpdate || stattree.IsExpanded(h_upload)) 
		{
			...
			if (forceUpdate || stattree.IsExpanded(h_up_session)) 
			{
				...
				cbuffer.Format(GetResString(IDS_STATS_ACTUL_ZZ),CGlobalVariable::uploadqueue->GetActiveUploadsCount()); //CGlobalVariable::uploadqueue->GetUploadQueueLength()
				stattree.SetItemText(up_S[2], cbuffer);

                // Set Set Total Uploads                
				cbuffer.Format(GetResString(IDS_STATS_TOTALUL),CGlobalVariable::uploadqueue->GetUploadQueueLength());
				stattree.SetItemText(up_S[3], cbuffer);

				// Set Queue Length
				cbuffer.Format(GetResString(IDS_STATS_WAITINGUSERS),CGlobalVariable::uploadqueue->GetWaitingUserCount());
				stattree.SetItemText(up_S[4], cbuffer);

				// Set Upload Sessions
				statGoodSessions = CGlobalVariable::uploadqueue->GetSuccessfullUpCount() + CGlobalVariable::uploadqueue->GetUploadQueueLength();
				statBadSessions = CGlobalVariable::uploadqueue->GetFailedUpCount();
				...
				if (forceUpdate || stattree.IsExpanded(up_S[5])) 
				{
					...
					// Set Avg Upload time
					//DWORD running=CGlobalVariable::uploadqueue->GetAverageUpTime();
					cbuffer.Format(GetResString(IDS_STATS_AVEUPTIME),CastSecondsToLngHM(CGlobalVariable::uploadqueue->GetAverageUpTime()));
			...
			if (forceUpdate || stattree.IsExpanded(h_up_total)) 
			{
				...
				// Upload Sessions
				statGoodSessions = CGlobalVariable::uploadqueue->GetSuccessfullUpCount() + thePrefs.GetUpSuccessfulSessions() + CGlobalVariable::uploadqueue->GetUploadQueueLength();
				statBadSessions = CGlobalVariable::uploadqueue->GetFailedUpCount() + thePrefs.GetUpFailedSessions();
				cbuffer.Format(_T("%s: %u"), GetResString(IDS_STATS_ULSES), statGoodSessions + statBadSessions);
				...
				if (forceUpdate || stattree.IsExpanded(up_T[1])) 
				{
					...
					// Set Avg Upload time
					uint32 avguptime = CGlobalVariable::uploadqueue->GetAverageUpTime();
	...
	{		
		...
		{			
			...
			if (forceUpdate || stattree.IsExpanded(hconn_sg)) 
			{
				...
				i++;
				// Active Connections
				cbuffer.Format(_T("%s: %i (%s:%u | %s:%u | %s:%u)"),GetResString(IDS_SF_ACTIVECON),CGlobalVariable::listensocket->GetActiveConnections(), GetResString(IDS_HALF), CGlobalVariable::listensocket->GetTotalHalfCon(), GetResString(IDS_CONCOMPL) ,CGlobalVariable::listensocket->GetTotalComp(), GetResString(IDS_STATS_PRTOTHER) ,CGlobalVariable::listensocket->GetActiveConnections() - CGlobalVariable::listensocket->GetTotalHalfCon() - CGlobalVariable::listensocket->GetTotalComp());
				stattree.SetItemText(conn_sg[i], cbuffer);
				i++;
				// Average Connections
				cbuffer.Format(_T("%s: %i"),GetResString(IDS_SF_AVGCON),(int)CGlobalVariable::listensocket->GetAverageConnections());
				stattree.SetItemText(conn_sg[i], cbuffer);
				i++;
				// Peak Connections
				cbuffer.Format(_T("%s: %i"),GetResString(IDS_SF_PEAKCON),CGlobalVariable::listensocket->GetPeakConnections());
				stattree.SetItemText(conn_sg[i], cbuffer);
				i++;
				// Connect Limit Reached
				uint32 m_itemp = CGlobalVariable::listensocket->GetMaxConnectionReached();
		...
		{			
			...
			if (forceUpdate || stattree.IsExpanded(hconn_tg)) 
			{
				...
				i++;
				// Average Connections
				cbuffer.Format(_T("%s: %i"), GetResString(IDS_SF_AVGCON), (int) (CGlobalVariable::listensocket->GetActiveConnections() + thePrefs.GetConnAvgConnections()) / 2 );
				stattree.SetItemText(conn_tg[i], cbuffer);
				i++;
				// Peak Connections
				...
				cbuffer.Format(_T("%s: %i"), GetResString(IDS_SF_MAXCONLIMITREACHED), CGlobalVariable::listensocket->GetMaxConnectionReached() + thePrefs.GetConnMaxConnLimitReached());
				stattree.SetItemText(conn_tg[i], cbuffer);
				i++;
			...
			for (int mx = 0; mx<3; mx++) 
			{
				if (forceUpdate || stattree.IsExpanded(time_aaph[mx])) 
				{
					...
					if (forceUpdate || stattree.IsExpanded(time_aap_hup[mx])) 
					{
						...
						// Upload Sessions
						uint32 statGoodSessions = (uint32)((CGlobalVariable::uploadqueue->GetSuccessfullUpCount() + thePrefs.GetUpSuccessfulSessions() + CGlobalVariable::uploadqueue->GetUploadQueueLength()) * avgModifier[mx]);
						uint32 statBadSessions = (uint32)((CGlobalVariable::uploadqueue->GetFailedUpCount() + thePrefs.GetUpFailedSessions()) * avgModifier[mx]);
	...
	if (forceUpdate || stattree.IsExpanded(h_clients)) 
	{
		...
		CGlobalVariable::clientlist->GetStatistics(totalclient, myStats, 
		...
		cbuffer.Format(_T("%s: %u"), GetResString(IDS_BANNED), CGlobalVariable::clientlist->GetBannedCount());
	...
	{		
		...
		cbuffer.Format(_T("%s: %s"),GetResString(IDS_SF_DELCOUNT),CastItoIShort(CGlobalVariable::serverlist->GetDeletedServerCount()));stattree.SetItemText(srv[2], cbuffer);
	...
	{		
		...
		cbuffer.Format(GetResString(IDS_SHAREDFILESCOUNT),CGlobalVariable::sharedfiles->GetCount());
		// SLUGFILLER: SafeHash - extra statistics
		if (CGlobalVariable::sharedfiles->GetHashingCount())
		{
			CString tempbuffer;
			tempbuffer.Format(GetResString(IDS_HASHINGFILESCOUNT),CGlobalVariable::sharedfiles->GetHashingCount());
			cbuffer += tempbuffer;
		}
		// SLUGFILLER: SafeHash
		...
		uint64 allsize=CGlobalVariable::sharedfiles->GetDatasize(bytesLargestFile); // Func returns total share size and sets pointeredd uint64 to largest single filesize
		CString cbuffer2;
		if(CGlobalVariable::sharedfiles->GetCount() != 0)
			cbuffer2.Format( _T("%s"), CastItoXBytes(allsize/(uint64)CGlobalVariable::sharedfiles->GetCount(), false, false));
		else 
	...
	{			
		...
		CGlobalVariable::downloadqueue->GetDownloadStats(myRateStats, 
