/* 
 * $Id: ClientDetailDialog.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
#ifdef _DEBUG_KAD_
        // VC-Huby[2006-11-24]: 增加Buddy信息显示
		if( client->HasLowID() )
		{
			TCHAR buddyInfo[50];
			if( client->GetBuddyIP() && client->GetBuddyPort() )
			{
				SetDlgItemText( IDC_STATIC30,s_addr_port(buddyInfo,client->GetBuddyIP(),client->GetBuddyPort()) );
			}
			else if( client->GetBuddyID() )
			{
				SetDlgItemText( IDC_STATIC30,md4str(client->GetBuddyID()) );
			}
			else
			{
				SetDlgItemText( IDC_STATIC30,_T("No Buddy Info") );
			}
		}
		else
		{
			SetDlgItemText( IDC_STATIC30,_T("High Id!") );
		};
		// VC-Huby[2006-11-24]:End
#endif
		CString buffer;
...
//#if defined(_DEBUG) || defined(_BETA) || defined(_VCALPHA)
...
#ifdef _SHOW_SOURCE_IP
		if( client->GetConnectIP() )
		{
			GetDlgItem(IDC_DSIP)->SetWindowText(ipstr(client->GetConnectIP()));
		}
		if( client->GetIP() )
		{
			GetDlgItem(IDC_DSIP)->SetWindowText(ipstr(client->GetIP()));
		}
		else
		{
		...
		if (client->GetServerIP()){
			...
			CServer* cserver = CGlobalVariable::serverlist->GetServerByIPTCP(client->GetServerIP(), client->GetServerPort());
			if (cserver)
...
#ifdef _SHOW_SOURCE_IP
	GetDlgItem(IDC_STATIC35)->SetWindowText( _T("Source IP:") );
#else
