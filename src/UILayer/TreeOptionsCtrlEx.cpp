/* 
 * $Id: TreeOptionsCtrlEx.cpp 4974 2008-03-12 08:41:01Z wangna $
 * 
...
CTreeOptionsCtrlEx::CTreeOptionsCtrlEx(CPropertyPage * ppgTweaks , UINT uImageListColorFlags)
{
	m_uImageListColorFlags = uImageListColorFlags;
	//SetAutoSelect(TRUE);	// does not work!?
	SetToggleOverIconOnly(TRUE);
	pPPgTweaks = (CPPgTweaks *)ppgTweaks;
}
...
void CTreeOptionsCtrlEx::HandleChildControlLosingFocus(bool bLosingFocus)
{
	CTreeOptionsCtrl::HandleChildControlLosingFocus(bLosingFocus);
}
...
void CTreeOptionsCtrlEx::MinMaxInt(HTREEITEM hItem,int minVal,int maxVal,CString orgVal)
{
		CString text;
		text.Format(GetResString(IDS_EDIT_RANGER),minVal,maxVal);
		MessageBox(text,GetResString(IDS_CAPTION),MB_OK|MB_ICONWARNING);
		SetEditText(hItem,orgVal);
}
void CTreeOptionsCtrlEx::MinMaxFloat(HTREEITEM hItem,CString minVal,CString maxVal,CString orgVal)
{
	CString text;
	text.Format(GetResString(IDS_EDIT_FLOAT_RANGER),minVal,maxVal);
	MessageBox(text,GetResString(IDS_CAPTION),MB_OK|MB_ICONWARNING);
	SetEditText(hItem,orgVal);
}
void CTreeOptionsCtrlEx::UpdateTreeControlValueFromChildControl(HTREEITEM hItem)
{
		if (m_pEdit)
		{
			CString sText;
			m_pEdit->GetWindowText(sText);      
			pPPgTweaks->m_ItemValueList.Lookup(hItem,pPPgTweaks->m_ItemValue);
			if (sText.IsEmpty())
			{
				SetEditText(hItem,pPPgTweaks->m_ItemValue.orgVal);
				return;
			}
	        if (pPPgTweaks->m_ItemValue.type == type_int)
	        {
		       __int64 iValue;
			   int minVal,maxVal,orgVal;
				minVal = _wtoi(pPPgTweaks->m_ItemValue.minVal);
				maxVal = _wtoi(pPPgTweaks->m_ItemValue.maxVal);
			   ...
			   iValue = _wtoi64(sText);
			   orgVal = _wtoi(pPPgTweaks->m_ItemValue.orgVal);
			   if (iValue < minVal || iValue > maxVal)
			   { 
				   MinMaxInt(hItem,minVal,maxVal,pPPgTweaks->m_ItemValue.orgVal);
			   }
			   else
			   {
			...
			else if (pPPgTweaks->m_ItemValue.type == type_uint)
			{
				uint64 iValue;
				UINT minVal,maxVal,orgVal;
				minVal = _wtoi(pPPgTweaks->m_ItemValue.minVal);
				maxVal = _wtoi(pPPgTweaks->m_ItemValue.maxVal);
				for(int i = 0;i < sText.GetLength(); i++)
				{
			...
			else if (pPPgTweaks->m_ItemValue.type == type_float)
			{
				double iValue,minVal,maxVal,orgVal;
				minVal = _wtof(pPPgTweaks->m_ItemValue.minVal);
				maxVal = _wtof(pPPgTweaks->m_ItemValue.maxVal);
				
				for(int i = 0;i < sText.GetLength();i++)
				{
					TCHAR cText = sText[i];
					if ((cText < '0' || cText > '9') && (cText != '.'))
					{
						CString text;
						text.Format(GetResString(IDS_EDIT_FLOAT_RANGER),pPPgTweaks->m_ItemValue.minVal,pPPgTweaks->m_ItemValue.maxVal);
						MessageBox(text,GetResString(IDS_CAPTION),MB_OK|MB_ICONWARNING);
						SetEditText(hItem,pPPgTweaks->m_ItemValue.orgVal);
						return;
					}
                ...
                iValue = _wtof(sText);
                orgVal = _wtof(pPPgTweaks->m_ItemValue.orgVal);
				if (iValue < minVal || iValue > maxVal)
				{ 
					MinMaxFloat(hItem,pPPgTweaks->m_ItemValue.minVal,pPPgTweaks->m_ItemValue.maxVal,pPPgTweaks->m_ItemValue.orgVal);
				}
				else
				{
					SetEditText(hItem,sText);
		   ...
		   CTreeOptionsCtrl::UpdateTreeControlValueFromChildControl(hItem);
}
