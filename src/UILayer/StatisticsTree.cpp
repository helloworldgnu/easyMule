/* 
 * $Id: StatisticsTree.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
CString CStatisticsTree::GetHTML(bool onlyVisible, HTREEITEM theItem, int theItemLevel, bool firstItem)
{
	...
	CString	strBuffer;
	if (firstItem)
		strBuffer.Format(_T("<font face=\"Tahoma,Verdana,Courier New,Helvetica\" size=\"2\">\r\n<b>eMule v%s %s [%s]</b>\r\n<br /><br />\r\n"), CGlobalVariable::GetCurVersionLong(), GetResString(IDS_SF_STATISTICS), thePrefs.GetUserNick());
...
void CStatisticsTree::ExportHTML()
{
	...
	if (saveAsDlg.DoModal() == IDOK)
	{
		...
		strHTML.Format( CString("<html>\r\n<header>\r\n<title>eMule %s [%s]</title>\r\n"
			...
			GetResString(IDS_SF_STATISTICS), GetResString(IDS_CD_UNAME), thePrefs.GetUserNick(), GetHTMLForExport() );
