/* 
 * $Id: PPgWebServer.cpp 4483 2008-01-02 09:19:06Z soarchin $
 * 
...
BOOL CPPgWebServer::OnInitDialog()
{
	...
	m_ttc.Create(this);
	m_ttc.AddTool(GetDlgItem(IDC_TMPLPATH), GetResString(IDS_TIP_WEBI_TEMPLATE));
	m_ttc.AddTool(GetDlgItem(IDC_TEMPLATE), GetResString(IDS_TIP_WEBI_TEMPLATE));
...
{	
	if(m_bModified)
	{
		...
		CGlobalVariable::webserver->ReloadTemplates();
		...
		if (_tstoi(sBuf)!=oldPort) {
			...
			CGlobalVariable::webserver->RestartServer();
		}
		...
		if (_tstoi(sBuf)!= thePrefs.GetMMPort() ) {
			...
			CGlobalVariable::mmserver->StopServer();
...
void CPPgWebServer::Localize(void)
{
	if(m_hWnd){
		...
		m_ttc.UpdateTipText(GetResString(IDS_TIP_WEBI_TEMPLATE), GetDlgItem(IDC_TMPLPATH));
		m_ttc.UpdateTipText(GetResString(IDS_TIP_WEBI_TEMPLATE), GetDlgItem(IDC_TEMPLATE));
