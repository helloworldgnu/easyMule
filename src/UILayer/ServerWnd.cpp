/* 
 * $Id: ServerWnd.cpp 7142 2008-09-08 11:00:22Z dgkang $
 * 
...
#define	LOG_PANE_RICHEDIT_STYLES WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_READONLY | ES_NOHIDESEL
	...
	if (servermsgbox->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_SERVMSG)){
		...
		servermsgbox->AppendText(GetResString(IDS_CAPTION) + _T(" v") + CGlobalVariable::GetCurVersionLong() + _T("\n"));
	...
	if (leecherlog->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_LEECHERLOG)){
		leecherlog->SetProfileSkinKey(_T("AntiLeecherLog"));
	...
	newitem.mask = TCIF_TEXT|TCIF_IMAGE|TCIF_PARAM;
	newitem.pszText = const_cast<LPTSTR>((LPCTSTR)name);
	newitem.iImage = 1;
	newitem.lParam = PaneServerInfo;
...
void CServerWnd::SetAllIcons()
{
	...
	iml.Add(CTempIconLoader(_T("Leecher")));
...
bool CServerWnd::AddServer(uint16 nPort, CString strAddress, CString strName, bool bShowErrorMB)
{
	...
	{
		CServer* pFoundServer = CGlobalVariable::serverlist->GetServerByAddress(toadd->GetAddress(), toadd->GetPort());
		if (pFoundServer == NULL && toadd->GetIP() != 0)
			pFoundServer = CGlobalVariable::serverlist->GetServerByIPTCP(toadd->GetIP(), toadd->GetPort());
		if (pFoundServer)
		{
...
void CServerWnd::OnBnClickedResetLog()
{
	...
	//{begin} VC-dgkang 2008年9月8日
	TCITEM itemSel;
	itemSel.mask = TCIF_PARAM;
	StatusSelector.GetItem(cur_sel,&itemSel);
    // {end}
	//Xman Anti-Leecher-Log
	if (itemSel.lParam == PaneLeecherLog)
	{
		theApp.emuledlg->ResetLeecherLog();
		theApp.emuledlg->statusbar->SetText(_T(""), SBarLog, 0);
	}
	//Xman end
	if (itemSel.lParam == PaneVerboseLog)
	{
		theApp.emuledlg->ResetDebugLog();
		theApp.emuledlg->statusbar->SetText(_T(""), SBarLog, 0);
	}	
	if (itemSel.lParam == PaneLog)
	{
		theApp.emuledlg->ResetLog();
		theApp.emuledlg->statusbar->SetText(_T(""), SBarLog, 0);
	}
	if (itemSel.lParam == PaneServerInfo)
	{
		servermsgbox->Reset();
...
void CServerWnd::ToggleDebugWindow()
{
	...
	if (thePrefs.GetVerbose() && !debug)
	{
		...
		newitem.iImage = 0;
		newitem.lParam = PaneVerboseLog;
		StatusSelector.InsertItem(2,&newitem); //InsertItem(StatusSelector.GetItemCount(),&newitem);
		debug = true;
	}
...
void CServerWnd::ToggleDLPWindow()
{
	int cur_sel = StatusSelector.GetCurSel();
	if (thePrefs.GetAntiLeecherLog() && !dlp)
	{
		TCITEM newitem;
		CString name;
	...
	else if(!thePrefs.GetAntiLeecherLog() && dlp)
	{
		if (cur_sel == PaneLeecherLog) //Xman Anti-Leecher-Log
		{
			StatusSelector.SetCurSel(PaneLog);
			StatusSelector.SetFocus();
		}
		...
		leecherlog->ShowWindow(SW_HIDE); //Xman Anti-Leecher-Log
		logbox->ShowWindow(SW_SHOW);
		if( StatusSelector.GetItemCount()==3 )
