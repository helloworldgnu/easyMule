/* 
 * $Id: CollectionCreateDialog.cpp 4830 2008-02-22 08:04:43Z soarchin $
 * 
...
void CCollectionCreateDialog::OnBnClickedOk()
{
	...
	if(!sFileName.IsEmpty())
	{
		...
		else
		{
			...
			{
				CKnownFile* pKnownFile = CGlobalVariable::knownfiles->FindKnownFileByPath(sFilePath);
				if(pKnownFile)
				{
					CGlobalVariable::sharedfiles->RemoveFile(pKnownFile);
...
void CCollectionCreateDialog::UpdateAvailFiles(void)
{
	...
	if(m_bSharedFiles)
		CGlobalVariable::sharedfiles->CopySharedFileMap(Files_Map);
	else
		CGlobalVariable::knownfiles->CopyKnownFileMap(Files_Map);
