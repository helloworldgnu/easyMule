/*
 * $Id: MeterIcon.cpp 5081 2008-03-21 06:57:42Z fengwen $
 * 
...
{// begin DrawIconMeter
	...
	RECT	rtBar;
	rtBar.left = (m_sDimensions.cx/m_nNumBars)*nPos+m_nSpacingWidth;
	rtBar.right = (m_sDimensions.cx/m_nNumBars)*(nPos+1);
	rtBar.top = m_sDimensions.cy-((nLevel*(m_sDimensions.cy-1)/m_nMaxVal)+1);
	rtBar.bottom = m_sDimensions.cy;
	...
	if(hOldPen == NULL)
		return false;
	if(!Rectangle(hDestDC, rtBar.left, rtBar.top, rtBar.right, rtBar.bottom))
	{// begin error check
		return false;
	}// end error check
	...
	if (nLevel>0) 
	if(!Rectangle(hDestDCMask, rtBar.left, rtBar.top, rtBar.right, rtBar.bottom))
	{// begin error check
		return false;
	}// end error check
